...
由于Spark单机包含在第2章介绍的Spark一般安装，你可以使用Spark安装来初始化自己机器上的本地Spark 单机集群。实际工作中，你可能希望启动不同的机器上的工作节点，但为了简单起见，我们提供了在单个机器上启动单机集群的代码。
...
```
![Figure 6-2. Spark单机网站](figures/6_2.png)
...
```
![Figure 6-3. Spark单机网络接口](figures/6_3.png)
Spark单机中只注册了一个工作节点。点击这个工作节点的链接，查看这个节点的细节，例如内存和处理器，如图6-4所示。
![Figure 6-4. Spark单机工作节点网络接口](figures/6_4.png)
...
![Figure 6-5. 运行`sparklyr`应用的YARN资源管理器](figures/6_5.png)
...
![Figure 6-6. 运行Spark和R的Mesos网络接口](figures/6_6.png)
...
![Figure 6-7. 运行Spark包的Cloudera管理器](figures/6_7.png)
...
![Figure 6-8. 启动亚马逊EMR集群](figures/6_8.png)
...
![Figure 6-11. 使用R启动Spark集群的IBM Watson](figures/6_11.png)
...
![Figure 6-13. 用RStudio和`sparklyr`初始化的Qubole集群](figures/6_13.png)
...
本章介绍了内部部署和云计算的历史和权衡，并介绍了Kubernetes这一提供跨本地部署或多个云提供商之间灵活性的有前途的框架。本章还引入了集群管理器（Spark Standalone、YARN、Mesos和Kubernetes），作为运行Spark集群应用所需的软件。本章简要介绍了本地集群提供商，如Cloudera、Hortonworks和MapR，以及主要的Spark云提供商：亚马逊、Databricks、IBM、谷歌、微软和Qubole。

虽然本章为理解当前集群计算趋势、工具和服务提供商处理大规模数据科学项目提供了坚实的基础，但它并没有提供一个全面的框架来帮助你决定选择哪种集群技术。相反，请使用本章作为概述和起点来寻找其他资源，以帮助找到最适合组织需要的集群。
...
in图10-1.
![Figure 10-1. H2O components with Spark and R](figures/10_1.png)
...
```
![Figure 10-2. The H2O Flow interface using Spark with R](figures/10_2.png)
...
![Figure 10-3. The Seven Bridges of Königsberg from the Euler archive](figures/10_3.png)
...
```
![Figure 10-4. High school ggraph dataset with highest PageRank highlighted](figures/10_4.png)
...
![Figure 10-5. A decision tree to predict job attrition based on known factors](figures/10_5.png)
...
![Figure 10-6. Layered perceptrons, as illustrated in the book Perceptrons](figures/10_6.png)
...
![Figure 10-7. Training using distributed systems based on OpenAI analysis](figures/10_7.png)
...
![Figure 10-8. The idealized human diploid karyotype showing the organization of the genome into chromosomes](figures/10_8.png)
...
coord_flip()
```
![Figure 10-9. Genomic importance analysis using variantspark](figures/10_9.png)
...
![Figure 10-10. Spatial operations available in geospark](figures/10_10.png)
...
Figure 11-1.
![Figure 11-1. Map operation when multiplying by 10](figures/11_1.png)
...
![Figure 11-2. Expected function signature in `spark_apply()` mappings](figures/11_2.png)
...
ggplot2::geom_point())
```
![Figure 11-3. Modeling over species in the iris dataset](figures/11_3.png)
...
```
![Figure 11-4. Ray tracing in Spark using R and rayrender](figures/11_4.png)
...
![Figure 11-5. Map operation when multiplying with context](figures/11_5.png)
...
![Figure 11-6. Data transformation between Spark and R](figures/11_6.png)
...
![Figure 11-7. Data transformation between Spark and R using Apache Arrow](figures/11_7.png)
...
![Figure 12-1. Working with Spark Streaming](figures/12_1.png)
...
![Figure 12-2. Monitoring a stream generating rows following a binomial distribution](figures/12_2.png)
...
![Figure 12-3. A basic Kaa workflow](figures/12_3.png)
...
data.
![Figure 12-4. A Kaa workflow using stream processors](figures/12_4.png)
...
![Figure 12-5. Progression of Spark reactive loading data into the Shiny app](figures/12_5.png)
...
![Figure 13-1. Extending Spark using the Spark API or Spark extensions, or writing Scala code](figures/13_1.png)
...
![Figure 13-2. R package structure when using Scala code](figures/13_2.png)
...
![Figure 13-3. Creating a Scala extension package from RStudio](figures/13_3.png)
...
![Figure A-1. The R Project for Statistical Computing](figures/A_1.png)
...
![Figure A-2. Java download page](figures/A_2.png)
...
![Figure A-3. RStudio download page](figures/A_3.png)
...
![Figure A-4. RStudio overview](figures/A_4.png)

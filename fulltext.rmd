---
title: "用R玩转Spark——大规模数据分析和建模完全手册"
author: "魏博"
date: "`r Sys.Date()`"
output: 
  word_document:
...
library(knitr)
library(formatR)
opts_chunk$set(tidy=T, message=FALSE, fig.align='center', eval=F,warning=F)
```
...
几位作者，Javier Luraschi，Kevin Kuo，Edgar Ruiz会展示如何将Spark和R结合起来进行大数据分析。本书涵盖相关的数据科学话题，聚类计算，甚至高级用户都会感兴趣的问题。

* 在Apache Spark环境下，使用R来分析，探索，转换，可视化数据。
* 构建统计模型来抽取信息并预测输出，自动化生产级的工作流程。
* 使用分布式计算技术在多台机器上进行分析和建模。
* 轻松使用Spark处理来自多个数据源和格式的大规模数据。
* 学习其它建模框架进行大规模图处理，地理空间分析和基因组学分析。
* 深入高级话题，包括定制转换，实时数据处理和创建定制化Spark扩展。


```
从编写了流行的sparklyr库发明者身上，学习如何在R项目中利用分布式计算的力量。这是成为R强力用户的进阶之道。
--Bryan Jonas，美国军事科学院数学系讲师
```


Javier Luraschi是大规模数据科学诸多程序包的发明者，包括`sparklyr`，`r2d3`，`pins`和`cloudml`。

Kevin Kuogou构建了机器学习库，并领导了KASA AI的开放保险研究。

Edgar Ruiz构建了企业级的数据解决方案工具，包括`dbplot`，`tidepredict`和`modeldb`。


-------

致Adrian，Clara，Julian，Max，Mila和Roman。

-------

# 序

Apache Spark是一个基于可扩展性而构建的分布式计算平台：Spark的API使来自多个数据源的输入变得很容易，并且使用多种编程语言和算法来处理输入进而构建数据应用程序。R是数据科学和统计学中最强大的语言之一，因此将R与Spark结合起来是非常有意义的。幸运的是，R丰富的语言特性使得从R调用Spark的API与在本地数据源上运行R类似。有了这两个系统的背景知识，你就能够在Spark中调用大量计算，或者从最喜欢的R编程环境中并行运行R代码。

本书详细探讨了在R中使用Spark的方法，重点介绍了`sparklyr`包，它支持`dplyr`和R社区已有的其它包。本书详细涵盖了主要用例，从使用Spark引擎查询数据到探索性数据分析、机器学习、R代码的并行执行和流式处理。它还对运行Spark和监视作业执行提供了自含的介绍。而作者正是介绍这个话题的合适人员。Javier，Kevin和Edgar一直参与`sparklyr`开发项目。我很兴奋地看到他们把这本关于使用Spark和R的清晰而集中的指南编排得这么好。

我希望你也喜欢这本书，并通过它来提升你的R代码能力，并将其与更广泛的Spark生态系统的功能连接起来。而且因为这里的所有基础模块都是开源的，所以你可以毫不犹豫地向开发人员反馈改进这些工具的建议。
...
在信息呈指数级增长的世界中，Apache Spark等先进工具为我们解决今天面临的许多相关问题提供了支持。从基于数据驱动的决策寻找改进方法的公司，到解决医疗、金融、教育和能源问题的研究机构，Spark比以往任何时候都能更快、更可靠地分析更多信息。

人们编写了各种各样的书籍来学习Apache Spark。例如，《Spark: The Definitive Guide》是一本综合性的书籍，《Learning Spark》是一本介绍性的书籍，旨在帮助用户熟悉和运行Spark（两者都来自O'Reilly）。然而，在撰写本书的时候，既没有使用R语言学习Apache Spark的书籍，也没有专门为R用户或潜在的R用户设计的书籍。


有一些在线资源可以学习R语言中的Apache Spark。最著名的是Spark.rstudio.com网站和Spark.Apache.org网站上的Spark文档。这两个网站都是很好的在线资源；但是，上边的内容并不打算让你从初学者开始阅读。它们都假设读者对Apache Spark、R和集群计算有一定的了解。

这本书的目的是帮助所有人通过R使用Apache Spark。另外，由于R编程语言的目的是为了简化数据分析，我们相信这本书也可以提供学习用Spark解决数据分析问题的最简单途径。第1章提供了一个介绍，帮助快速理解这些概念，并介绍了在自己的计算机上处理这些问题所需的工具。然后，我们会快速进入相关的数据科学主题、集群计算和高级主题。即使是最有经验的用户也会感兴趣。

因此，本书旨在成为一个对广大用户有用的资源，从渴望学习Apache Spark初学者，到寻求理解为什么和如何通过R使用Apache Spark的有经验的读者。

本书的结构如下：

* 介绍
在前两章，第1章简介和第2章开始中，你将了解Apache Spark，R以及使用Spark和R执行数据分析的工具。
* 分析
在第3章分析中，你将学习如何使用R在Apache Spark中分析、探索、转换和可视化数据。
* 建模
在第4章建模和第5章管道中，你将学习如何创建统计模型，以便在生产环境的工作流中提取信息、预测结果和自动化过程。
* 规模化
到目前为止，本书的重点是在你的个人电脑上执行操作和接受有限的数据格式。第6章集群，第7章连接，第8章数据，以及第9章调试，介绍了跨多台机器和数据格式执行分析和建模所需的分布式计算技术，以解决Apache Spark关注的大规模数据和计算问题。
* 扩展
第10章扩展，描述了适用于特定用例的可选模块和扩展功能。你将学习其它建模框架、图处理、深度学习的数据预处理、地理空间分析和大规模基因组学。
* 高级技术
本书结尾是介绍一系列高级技术的章节，包括第11章分布式R，第12章Streaming，第13章社区贡献；这些内容是高级用户最感兴趣的。但是，当你学到本节时，你会发现其实并没有那么吓人；相反，这些章节将与前面的章节一样相关、有用和有趣。


第一组章节，1-5，简单地介绍了如何大规模执行数据科学和机器学习项目。如果你打算阅读这本书，同时也跟随代码示例，这些是可以逐行执行代码的好章节。因为这些章节介绍了个人电脑上涉及的所有概念。你不会利用多台电脑，而Spark却是为使用多台电脑而设计的。但是别担心：下一章将详细介绍这一点！

第二组章节，6-9，介绍了使用Spark进行集群计算中令人激动的基本概念。事实上，它们还介绍了集群计算中一些不那么有趣的部分。但是相信我们，这些概念还是值得学习的。此外，每章中的概述部分都特别有趣、信息丰富、易于阅读，有助于你直观地了解集群计算是如何工作的。对于这些章节，我们实际上并不建议逐行执行代码，尤其是在尝试从头到尾学习Spark的情况下。在拥有一个合适的Spark集群之后，你总是可以回过头来执行代码。但是，如果你已经有一个可以工作的集群，或者你真的很想得到一个集群，那么你可能需要借助第6章来选择一个集群，然后按照第7章来进行连接。

第三组章节，10-13，展示了对大多数读者来说应该很有趣的工具，并且会使后续的操作更容易。这里有很多高级主题，一些主题自然比其它主题更有意思。例如，你可能对分析地理数据集感兴趣，或者你对处理实时数据集更感兴趣，或者你希望两者兼得！基于个人兴趣或手头的问题，我们鼓励你执行最相关的代码示例。这些章节中的所有代码都是为了在个人计算机上执行而编写的，但我们也鼓励你使用合适的Spark集群，因为你会拥有解决问题和优化大规模计算所需的工具。
...
表的维度（行数和列数）在第一行中有描述，第二行是列名，第三行是列类型。我们在本书中使用的`tibble`包还提供了各种细微的视觉改进。

大多数绘图都使用`ggplot2`包和附录中提供的自定义主题进行渲染。但是，由于本书不关注数据可视化，因此我们只提供基本的绘图代码，其可能与应用的格式不匹配。如果你有兴趣学习R中更多关于可视化的知识，可以考虑专门的书籍，比如《R Graphics Cookbook》（O'Reilly）。

## 致谢

感谢那些允许通过R使用Spark的程序包作者：Javier Luraschi、Kevin Kuo、Kevin Ushey和JJ Allaire（`sparklyr`）；Romain franois和Hadley Wickham（`dbplyr`）；Hadley Wickham和Edgar Ruiz（`dpblyr`）；Kirill Mülller（`DBI`）；以及Apache Spark项目本身的作者及其最初作者Matei Zaharia。

感谢为丰富Spark和R生态系统而发布扩展的程序包作者：Akhil Nair（`crassy`）；Harry Zhu（`geospark`）；Kevin Kuo（`graphframes`、`mleap`、`sparktf`和`sparkxgb`）；Jakub Hava、Navdeep Gill、Erin LeDell和Michal Malohlava（`rsparkling`）；Jan Wijffels（`Spark.sas7bdat`）；Aki Ariga（`sparkavro`）；Martin Studer（`sparkbq`）；Matt Pollock（`sparklyr.nested`）；Nathan Eastwood（`sparkts`）；Samuel Macêdo（`variantspark`）。

感谢我们出色的编辑Melissa Potter为我们提供指导、鼓励和无数小时的详细反馈，使这本书成为我们的著作中最好的一本。

感谢Bradley Boehmke，Bryan Adams，Bryan Jonas，Dusty Turner和Hossein Falaki，感谢你们的技术评论、及时和坦率的反馈，并与我们分享专业知识。多亏了你们，许多读者才会有更愉快的阅读体验。

感谢RStudio、JJ Allaire和Tareef Kawaf对这项工作的支持，感谢R的持续支持和鼓励。

Max Kuhn，感谢你对第4章的宝贵反馈。在他的允许下，我们采用了他的精彩著作《特征工程与选择：预测模型的实用方法》（CRC出版社）中的例子。

我们也感谢所有间接参与但未在本节中明确列出的人们；我们真正站在了巨人的肩膀上。

这本书是用Yihui Xie的`bookdown`，JJ Allaire和Yihui Xie的`rmarkdown`，Yihui Xie的`knitr`写成的；用Hadley Wickham和Winston Chang的`ggplot2`完成可视化；用Daniel Kallin和Javier Luraschi的`nomnoml`绘制图表；用John MacFarlane的`pandoc`进行文档转换。
...
补充材料（示例代码、练习等）可在https://github.com/r-spark/the-r-in-spark上下载。

这本书是来帮你完成工作的。一般情况下，如果本书提供了示例代码，你就可以在你的程序和文档中使用。你不需要为了获得许可联系我们，除非你正在复制代码的重要部分。例如，编写一个使用本书中多段代码的程序不需要获得我们的许可。出售或分发O'Reilly书籍的示例光盘就需要获得许可。引用本书以及示例代码来回答问题也不需要许可。将本书中大量的示例代码合并到你的产品文档中就需要获得许可。

虽然我们会感谢你的引用，但是我们也不强制要求你这么做。引用通常包括标题、作者、出版商和ISBN。例如：“Mastering Spark with R by Javier Luraschi, Kevin Kuo, and Edgar Ruiz (O’Reilly). Copyright 2020 Javier Luraschi, Kevin Kuo, and Edgar Ruiz, 978-1-492-04637-0.”

如果你觉得你对代码示例的使用超出了合理使用或上述许可范围，请通过permissions@oreilly.com与我们联系。
...
随着信息以指数的速率增长，历史学家把这段历史时期称作信息时代也就不意外了。逐步加速的数据收集速度也创造了很多新的机会，并且必然会创造更多的机会。本章介绍一些用于解决大规模数据挑战的工具。首先，作为处理大规模数据集的流行工具，本章会介绍Apache Spark。有了这一基础，我们会介绍R编程语言，它是专门用于简化数据分析的。最后，我们会介绍`sparklyr`，该项目将R和Spark结合成为大家都可使用的强大工具。

*第2章 开始*会介绍用于在个人电脑上执行Spark和R运算所需的预备知识，工具和步骤。你会学到如何安装和初始化Spark，了解常见操作，处理第一份数据和完成建模任务。本章的目的就是要帮助读者掌握处理大规模数据问题所需的概念和工具，而这些问题目前只有少数组织可以接触到。

然后，你可以继续学习如何分析大规模数据，以及构建模型支持预测趋势和发现隐藏在海量信息中的知识。接着，你可以掌握一些用于执行数据分析和大规模建模的工具。后续章节会帮助你从本地计算机中跳出来，转向计算集群，以便解决许多真实世界的问题。最后几章会介绍一些其它的话题，例如实时数据处理和图分析，可以帮助你真正的掌握任何规模数据分析的最先进技能。本书的最后一章会介绍一些工具和想法，读者可以考虑在Spark和R社区中做些贡献。
...
世界银行发布的关于数字发展的报告给出了过去三十年保存下来的数字信息和模拟信息总量估计[^3]。这份报告指出，数字信息大概在2003年超过了模拟信息，其总量有大约一千万太字节，大约相当于今天的一千万存储驱动。然而，这份报告中更相关的发现是，我们的数字信息正在以指数的速度增长。图1-1展示了这份报告的结论。注意，每隔一年，世界上的信息都会增长10倍。

[^3]: Group WB (2016). The Data Revolution. World Bank Publications.

为了提供工具找出所有新的数字信息，许多公司都试图提供相关的功能。例如，我们今天熟知的用于搜索网络的搜索引擎。给定海量的数字信息，管理如此规模的信息绝对是一项挑战。搜索引擎并不能存储所有的网页信息，支持在单一电脑上搜索网页。这就意味着它必须把信息分成几个文件，并存储在多个机器上。这项技术就是*谷歌文件系统（Google File System）*，它在谷歌2003年发表的学术文献中有介绍[^4]。
...
![图1-1. 全球信息存储能力](figures/1_1.png)

图中文字：

* World。。。Growth，全球信息增长趋势
* Storage。。。terabytes，每年存储能力，太字节为单位
* terabytes，太字节
* analog，模拟信息
* digital，数字信息
...
一年以后，谷歌发表了新的文章，描述了如何在谷歌文件系统上执行操作，也就是后来的*MapReduce*技术。[^5]。正如你可以预见到的，MapReduce中有两种操作：map和reduce。*map*操作提供任意一种方法把每个文件转化为一个新的文件，而*reduce*操作可以合并两个文件。两种操作都需要定制计算机代码，而MapReduce框架关注一次性在多个电脑上的自动执行。这两个操作足够支持网络上所有数据的处理，同时也提供足够的灵活性从数据中得到有意义的信息。
...
例如在图1-2中，我们可以使用MapReduce计算存放在不同机器上的两个不同文本文件的词语数量。map操作切分原始文件中的每个词语，并输出一个新的词语计数文件，其包含词语到计数的映射。reduce操作可以定义为对两个词语计数文件的合并，其通过整合每个词语的计数总数来实现。最终的文件会包含来自所有原始文件的词语计数。
...
* Alice's file, Alice的文件
* Alice's words Alice的词语
* Bob's file, Alice的文件
* Bob's words Alice的词语
* Word counts，词语计数

在谷歌发表了这些文献之后，雅虎的一个团队致力于将谷歌文件系统和MapReduce作为一个单一的开源项目。这个项目就是2006发布的*Hadoop*，其中谷歌文件系统通过*Hadoop分布式文件系统（Hadoop Distributed File System，HDFS）*实现。Hadoop项目使得基于分布式文件的计算可以被更多的用户和组织获取，这也使得MapReduce的能力突破了网络数据处理的限制。

尽管Hadoop提供了在分布式文件系统上执行MapReduce操作的支持，但是它依然需要在每次数据分析的时候写好MapReduce操作代码。为了改进这一繁琐的过程，Facebook于2008年发布了Hive项目。它给Hadoop引入了*结构化查询语句（Structured Query Language，SQL）*。这意味着大规模数据分析不再需要为每个MapReduce操作编写代码。相反，用户可以用SQL编写通用的，更加容易理解和编写的数据分析语句。

## Spark

2009年，Apache Spark开始作为加州大学伯克利分校AMP实验室的科研项目，以期改进MapReduce。Spark针对性的提供了更加丰富的MapReduce以外的运算符集合，便于优化多个机器上的代码运行。Spark同时将数据加载到内存中，使得各种操作要比Hadoop的磁盘存储快很多。最新的一项结果显示，运行逻辑回归，一种在第4章中介绍的数据建模技术，利用内存中数据集的Spark要比Hadoop快10倍[^6]。类似于图1-3的结果在原始研究文献中也有展示。
...
![图1-3. 逻辑回归在Hadoop和Spark上的性能](figures/1_3.png)
图中文字：

* Logistic。。。time，逻辑回归执行时间
* seconds，秒数
* iterations，迭代次数

尽管Spark以内存操作的性能而闻名，而当初设计时是作为支持内存和磁盘操作的一种通用执行引擎。例如，Spark创造了新的大规模排序记录，而这些数据并没有存在内存中。相反，Spark改进了网络序列化，网络改组，CPU缓存的高效利用，进而极大的提高了性能。如果你需要对大规模数据进行排序，世界上没有比Spark更快的系统了。

为了让你了解Spark有多快多高效，Hadoop使用了72分钟，对2100台计算机完成了100太字节数据的排序，但是Spark只用了23分钟和206台计算机。另外，Spark拥有云端排序记录，使其成为云端对大规模数据进行排序的最具性价比的解决方案。
...
|节点数量| 2,100 |206|
|内核数量| 50,400 |6,592|
|磁盘| 3,150GB/秒| 618GB/秒|
|网络| 10GB/秒| 10GB/秒|
|排序速度| 1.42TB/分钟 |4.27TB/分钟|
|排序速度/节点| 0.67GB/分钟| 20.7GB/分钟|


Spark也比Hadoop易用。例如，词语计数的MapReduce示例在Hadoop中需要50行代码，而在Spark中只需要2行代码。正如所见，和Hadoop相比，Spark更快，更高效，更易用。

2010年，Spark作为开源项目发布出来，并于2013年捐赠给了Apache软件基金会。Spark执行Apache 2.0协议，允许用户免费使用，修改和分发。Spark有超过1,000名贡献者，是Apache软件基金会中最活跃的项目。
...
Spark可以看做是一种*引擎*，因为它是通用的和高效的。它的通用性是因为Spark可以优化和执行通用代码。也就是说，对于Spark中的程序代码并没有什么类型限制。它的高效性是因为Spark可以有效利用内存，网络和CPU加速计算集群中的数据处理算法。正如之前提到的，它比其它技术都要快速。

这些特点使得Spark在许多分析任务中，例如Netflix的电影排序，蛋白质序列对齐，和CERN的高能物理分析中，成为理想工具。

作为一种*统一*的平台，Spark可以支持许多集群技术和多种数据源。你可以分别在第6章和第8章中了解到。它也支持许多不同的库，例如Spark SQL，MLlib，GraphX和Spark Streaming，分别支持分析，建模，图处理和实时数据处理。总之，Spark是一种支持请求集群，数据源和各种库的平台，进而实现大规模计算，如图1-4所示。
...
* 高速性
    ...
    假设你有一个10GB的数据集，它的数据处理过程需要30分钟。这既不是一个大数据问题也不是一个大计算问题。但是，如果你突发奇想要改进模型的准确率，减少运行时间到3分钟以内，这就是一个显著的改进。这种数据分析速度的改进可以带来有意义的推进和生产力的提高。另外，你也可能需要更快的处理数据速度，比如支持股票交易。尽管三分钟已经足够快，但是这对于实时数据处理依然很慢。这种场景下，你可能需要在几秒内甚至几毫秒内完成数据处理。

* 多样性
  
    你可能有了一个高效的数据收集流程，从多个数据源收集数据放在一个位置，通常是一个数据库。这个过程可能已经非常高效并且接近实时。这样的过程就是通常所说的*抽取，转换和加载（Extract，Transform，Load，ETL）*；数据从多个数据源抽取，转换成所需的格式，加载到单个数据存储结构中。尽管这一方法已经流行了多年，但是其中的权衡是添加新的数据源代价很高。因为系统是集中控制的，并且是严密操作的，任何改变都可能使整个过程暂停；因此，添加新的数据源通常要花费很长的时间才能实现。相反，你可以把全部数据按照自然格式存放，并且按照需要使用集群处理，即数据湖的架构。另外，按照自然格式存放数据支持处理图片，声音，和视频格式的新文件，而不需要明确如何把它们放进传统的结构化存储系统。

* 真实性

    当使用多个数据源的时候，你可能发现它们之间的数据质量参差不齐，需要专门的分析方法来改进准确性。例如，假设你有一个城市列表，取值包括旧金山，西雅图，和波士顿。当数据包含拼错的单词，例如“Bston”，会发生什么呢？在传统的关系型数据库中，这类非法输入可能会被舍弃。但是，舍弃取值并不一定在所有情形下都是最好的方法；你会希望利用地理编码，交叉验证数据源，或者尽最大努力匹配来纠正这些字段。所以，理解和改进原始数据源的真实性可以带来更准确的结果。

如果我们把“海量性”当做是大数据的同义词，你就得到了方便记忆的大数据的4个V。一些人也把它扩展成5个V甚至10个V。除了几个便于记忆的符号，今天集群计算也正在以更加创新的方式使用。看到各个组织实验新的工作流，以及实验传统意义上并不适用集群计算的各种任务，已经不再稀奇。更多的源于大数据的热潮都是这类问题。严格意义上讲，你不是在解决大数据，但是依然可以从大数据和大计算工具的使用中获益。

我们希望本书可以帮助你理解集群计算的机遇和局限，特别是使用Apache Spark和R的机遇和局限。

## R

R编程语言源于诞生在贝尔实验室的S语言。Rick Becker在2016年的useR!会议上解释，在当时的贝尔实验室里，计算是通过调用Fortran语句写成的子程序而实现的，而这种程序显然不好处理。而S计算语言当时是作为一种接口语言而设计的，它可以解决特殊的问题，而不需要使用其它语言，例如Fortran。S语言的发明者，John Chambers在图1-5中介绍了S是如何设计出来的，进而提供简化数据处理的接口。它的共同发明者也在2016年的useR!上展示了这一原图，启发了S的设计。

![图1-5. John Chambers的接口语言图 (Rick Becker useR! 2016)](figures/1_5.png)

R语言是S语言的一种现代而自由的实现。特别的，根据《R Project for Statistical Computing》：
...
* R语言
    ...
    R是由统计学家设计的，也是面向统计学家使用的。这意味着R是少有的，成功的面向非编程人员的编程语言之一。所以学习R会更加自然。另外，因为R语言是其它语言和工具的接口，所以R可以让你更多的关注理解问题，而更少的花在计算机科学和工程上。

* R社区

    R社区通过*R语言综合文档网络（Comprehensive R Archive Network，CRAN）*提供了丰富的程序包，并支持安装即时可用的程序包，来执行各种任务——最享有盛名的高质量数据操作，可视化，和统计模型。许多操作只能在R中进行。另外，R社区是一个热情的，活跃的组织，这些优秀的朋友都愿意帮助你成功。R社区提供的许多程序包，到目前为止，都是统计计算的最优选择。一些下载最多的程序包包括：操作数据的`dplyr`，分析聚类的`cluster`，可视化数据的`ggplot2`。通过画出每天CRAN里R程序包的下载量，图1-6定量刻画了R社区的增长。

![图1-6. CRAN程序包每日下载量](figures/1_6.png)

图中文字：
* CRAN packages，CRAN程序包
* Total。。。time，每日下载量随时间变化
* downlowds，下载量

除了统计分析，R也用在其它领域。以下领域和本书高度相关：
...
处理上述几个领域时，你要么会面对逐渐增大的数据集，要么会面对逐渐复杂的计算过程。这些流程执行起来都很慢，或者某些时候在单台计算机上变得不可能。但是，要知道，Spark并不一定是所有计算问题的答案。面对R中的计算问题时，使用以下技术会很有效：
...
* 代码分析
    ...
    你可以自己试着理解为什么计算很慢，并且需要改进。分析器是检查代码执行情况，帮助识别瓶颈的工具。在R中，R的分析器，`profvis`程序包和RStudio的分析器功能可以让你轻松的重试和可视化一个分析器。然而，优化代码通常并不容易。

* 纵向扩展

    加速计算通常可以通过购买更快或者更强劲的硬件（即提高机器内存，升级硬盘，或者使用CPU更多的计算器）来实现。这些手段叫做纵向扩展。但是，对于单个计算机的纵向扩展通常存在硬极限，即使使用大量的CPU，你还需要新的框架来有效地并行化计算。

* 横向扩展

    最后，我们可以考虑把计算和存储扩大到多台机器上。这种手段提供了最高维度的扩展性能。因为你可以使用任意数量的机器来执行计算。这个技术通常叫做横向扩展。但是，有效的扩展计算是一项复杂的实践，特别是没有使用专门的工具和框架，例如Apache Spark的时候。

最后一点更加具体的介绍了本书的目的，即Apache Spark提供的分布式计算系统的能力，借助R解决数据科学和相关领域中有意义的计算问题。

## `sparklyr`

考虑到Spark提供的计算能力和R语言的易用性，我们很自然的想让二者无缝的结合起来。这也是R社区期望的：提供Spark接口的R程序包用起来很简单，并与其它R程序包兼容，而且可以在CRAN上找到。为此，我们开始开发`sparklyr`。第一版，`sparklyr 0.4`，在2016年的useR!大会上发布。第一版包括对`dplyr`，`DBI`，建模工具MLlib的支持，以及支持类似H2O的`rsparkling`程序包的可扩展API。 从此，许多新的功能和改进在`sparklyr 0.5, 0.6, 0.7, 0.8, 0.9`和`1.0`做了实现。

根据官方定义，`sparklyr`是Apache Spark的R语言接口。它可以像其它CRAN程序包一样工作通过CRAN获取。这意味着人们无需知道Spark的版本，就可以完成安装，同时服务于R社区，兼容其它R程序包，并接受来自R社区的使用反馈，等等。它托管在GitHub，遵循Apache 2.0协议，允许克隆，修改，并用户参与项目开发。使用`sparklyr`的用户应该包含以下角色：
...
* 数据科学家
    ...
    对于已经使用和钟爱R的数据科学家，`sparklyr`整合了许多其它的R实践和程序包，例如`dplyr`，`magrittr`，`broom`，`DBI`，`tibble`，`rlang`，以及其它框架，让你使用Spark从容自如。而对于R和Spark的新手，`sparklyr`的高级别工作流整合和低级别扩展机制保证Spark是一个高产的环境，可以满足每个数据科学家的需求和技能。

* 专家用户

    对于已经深度使用Spark，并可以编写Scala原生代码的用户，可以考虑让你的Spark程序包作为一个R程序包贡献给R社区。一个多样的，强大的社区可以让你的工作贡献出最好的价值，促进开源社会的发展。

我们编写这本书是为了介绍和传授Apache Spark和R融合的部分。`sparklyr`就是把这些社区，期望和未来方向，程序包及其扩展整合在一起的R程序包。我们相信，使用这本书可以搭起R和Spark社区的桥梁：给R社区介绍为什么Spark让人兴奋，给Spark社区介绍为什么R如此伟大。两个社区都在使用不同的技术和背景来解决相似的问题。因此，我们希望`sparklyr`成为创新的肥沃土壤，新手的好客之地，是数据科学达人的高产环境，也是集群计算，数据科学，机器学习一起交流的开放社区。

## 小结

本章介绍了Spark这一现代而强大的计算平台，R这一易用的，基于扎实的统计建模理论的计算语言，以及`sparklyr`这一连接两个技术和社区的项目。我们处在一个信息指数级增长的世界，学习如何分析大规模数据可以帮助你解决当今人类面临的问题和机遇。但是，在分析数据之前，第2章会介绍本书后面内容所需的工具。确保你可以明白每一步操作，并花点时间安装推荐的工具，因为它们会成为你经常使用和喜爱的资源。
...
完成第1章的学习，你应该熟悉了Spark可以解决的各类问题。显而易见，Spark利用多台计算机来解决数据无法在单个机器上运行或者计算太慢的问题。如果你是R的新手，也可以看到，把Spark和数据科学工具，例如可视化工具`ggplot2`，数据变换`dplyr`，结合起来可以给大规模数据科学项目带来美好的想象。我们也希望你因为自己成为大规模数据计算的高手而感到兴奋。

在本章中，我们会简单介绍一下成为Spark高手所需要的工具。我们鼓励你能够实现本章的代码，因为这样会让你理解分析，建模，读取和写入数据的各种动作。换句话说，你需要坚持不懈，在使用Spark前反复实践。

在第3章中，我们会深入介绍分析和建模，并给出在单个集群上，也就是你的个人电脑上运行的例子。后续章节介绍集群计算，以及成功在多台机器上运行代码所需的概念和技术。
...
完成预备操作后，你首先要学习如何连接Spark。我们会介绍本书后续章节中用到的最重要的工具和操作。我们不会过多关注教授概念或者如何使用概念。我们也不可能在一章中解释完建模或流式计算。但是，完成本章的学习让你窥见到Spark的能力，以及确保工具已经完成正确的部署，足以应付未来更多棘手的问题。

所用的工具大部分都分成R代码和Spark网络接口。所有Spark操作都在R中运行。但是，监控分布式操作的行文会在Spark的网络接口中操作，你可以通过任何一个网络浏览器加载看到。然后我们断开本地集群的连接。虽然这很容易忘记，我们还是强烈建议在使用本地集群和共享集群的时候能够记得这一操作.
...
你也可以运行`Sys.setenv(JAVA_HOME = "path-to-java-8")`，使用`JAVA_HOME`环境变量来指定具体的Java版本。也可以在安装`sparklyr`之前，确保R环境可以使用Java 8版本。

### 安装`sparklyr`

和许多其它的R程序包一样，你可以使用如下命令安装CRAN上的`sparklyr`：

```{r}
install.packages("sparklyr")
```

本书的示例假设你使用最新版本的`sparklyr`。你可以运行如下代码，验证你的版本是不是和我们的一样新：
...
这个代码可以实现R中所有`sparklyr`函数，非常有用。否则，你需要带着前缀`sparklyr::`运行每一个`sparklyr`命令。
...
你可以使用Spark版本信息安装特定的版本，或者也可以指定Hadoop版本。例如，要安装Spark 1.6.3，你可以运行：
...
> 如果你使用的是自己的或者在线的Spark集群，确保你是按照集群管理员的配置或者在线文档进行的连接。如果你需要指导，可以参考第7章，它会详细介绍如何连接Spark集群。
...
如果连接失败了，第7章包含有故障排查的内容，可以帮助你解决连接问题。
...
让我们看看`copy_to()`做了什么操作。第一个参数`sc`，给函数提供了一个工作的Spark连接指向，这个连接是之前通过`spark_connect()`创建的。第二个参数给出了要加载到Spark的数据集。现在，`copy_to()`返回Spark中数据集，并由R环境自动打印。只要Spark打印数据集，它就会收集几条数据记录并在前台展示。在这个例子中，数据集只包含几行描述机动车型号的信息以及诸如马力，和每加仑可行驶英里数的说明。
...
打印`cars`数据集会收集几行记录并展示到R控制台。你可以在Spark网络接口中看到，有一个作业用于在Spark中收集这个信息。你可以选择Storage标签，查看Spark内存中的`mtcars`数据集，如图2-2所示。
...
Executors标签，如图2-3所示，提供了集群资源的总览。对于本地连接，你只能看到一个2GB内存，384MB计算内存的Spark工作节点。在第9章中，你会看到如何请求更多的计算实例和资源，以及内存如何分配。

![图2-3. Apache Spark网络接口的Executors标签](figures/2_3.png)

最后一个要介绍的标签是Environment标签，如图2-4所示；这个标签列出了所有Spark应用的设置，我们会在第9章中介绍。你可以看到，大部分设置不需要明确的配置，但是要在大规模数据上顺利运行，你还是要熟悉这些配置。

![图2-4. Apache Spark网络接口的Environment标签](figures/2_4.png)

然后，你可以使用小的数据子集进行练习。我们会在第3章中详细介绍。

### 分析

在R中使用Spark分析数据时，可以使用SQL（结构化查询语句）或者`dplyr`（数据操作语法）。你可以通过`DBI`程序包使用SQL。例如，要计算我们的`cars`数据集中有多少记录，我们可以运行如下代码：
...
使用`dplyr`的时候，需要的代码更少。而且通常比SQL容易的多。这也是为什么本书不用SQL的原因。但是，如果你对SQL很熟悉，SQL也是备选方案。例如，`dplyr`中计算行数要更简洁，且易于理解：
...
一般情况下，我们通常首先使用`dplyr`在Spark中做数据分析，然后进行采样和列维度的子集选取。最后一步是从Spark中收集数据，并在R中做进一步数据处理，例如数据可视化。让我们运行一个非常简单的数据分析例子，包括对Spark中`cars`数据集选取，采样，绘制图形:
...
* Vehicle Effiency，车辆效率
* miles。。。galon，每加仑英里数
* horsepower，马力

### 建模

尽管数据分析可以让你快速理解数据，但是构建数学模型描述和泛化数据集也非常有用。在第1章中，你学习了机器学习和数据科学领域使用数学模型进行预测和发现重要的观点。例如，我们可以使用线性模型估计燃料效率和马力的关系：
...
尽管前面的例子缺少建模中所用的诸多技术，但是也足以作为一个简单的例子简要介绍Spark的建模能力。我们会在第4章中介绍所有的Spark模型，技术和最佳实践。

### 数据

为了简单起见，我们把`mtcars`数据集复制给了Spark。但是，一般情况下数据通常不在Saprk中。相反，数据要从已有的数据源中读取，格式也多种多样，例如纯文本，CSV，JSON，Java数据库连接JDBC和其它。我们会在第8章中详细介绍这些内容。例如，我们可以把`cars`数据集输出为CSV文件：

```{r}
spark_write_csv(cars, "cars.csv")
```

实际工作中，我们会从分布式存储系统例如HDFS中读取已有的数据集，但是我们也可以从本地文件系统中读取数据：
...
R以其活跃的程序包作者社区而著名。同样的，许多Spark和R扩展已经被编程实现，并随时可用，只是数量较R社区中的少些。第10章会介绍许多有趣的程序包，它们支持高级建模，图分析，深度学习数据预处理等等。
...
尽管嵌套数据不太容易读取，但是在借助`spark_read_json()`和`spark_write_json()`函数处理诸如JSON的嵌套数据格式的时候，确实解决了这样的需求。
...
要体验Streaming，首先创建一个*input/文件夹*，放入一些数据流的输入：
...
然后，定义一个作业流，处理来自*input/*文件夹的数据，执行R代码定义的转换，并输出到一个*output/*文件夹：
...
实时数据一开始输入，*input/*文件夹几乎同时开始处理，并在*output/*文件夹中存入新的转换数据文件。因为输入文件夹只有一个文件，输出文件夹也只包含一个经过`spark_apply()`转换的文件。
...
到目前为止，这些操作和静态数据的处理很像。但是，我们可以持续添加*input/*文件夹中的文件，Spark会自动的并行处理数据。让我们添加一个新的文件，检查是否可以自动处理：
...
你可以使用`dplyr`，SQL，Spark模型，或者分布式R分析实时数据流。在第12章中，我们会详细介绍分析实时数据所需的所有有意思的转换。
...
我们也可以获取具体的日志记录，比如包含`sparklyr`的记录。使用`filter`参数，运行如下代码：
...
首先，不需要Rstudio控制台的`spark_connect()`建立连接，你可以用Connections标签下的New Connection操作，然后选取Spark connection，打开图2-7的对话窗口。你可以定制版本号和连接配置，生成正确的`spark_connect()`命令，并在R控制台中执行。
...
* Spark UI
    ...
    打开Spark网络接口，`spark_web(sc)`的快捷操作。

* 日志

    打开Spark网络日志，`spark_log(sc)`的快捷操作。
    
* SQL

    打开新的SQL查询。更多关于`DBI`和SQL支持的信息，可以查看第3章。
...
* 断开
    ...
    断开Spark连接，`spark_disconnect(sc)`的快捷操作。
    
本书后边的章节会使用纯R代码。本书中的代码也都在R环境中运行。是在R控制台，Rstudio，Jupyter Notebooks，还是其它支持R代码的工具中运行R代码，取决于你的个人需要。

## 资源

我们已经花了很大的精力简化上手过程，不过还有许多其它资源可以帮助你解决具体的上手问题。通常，这些资源会介绍更广泛的Spark和R社区，帮助你找到具体的答案，讨论某个话题，以及与经常使用Spark和R的用户沟通：

* 文档

    托管在RStudio的Spark网站文档站点应该是了解更多在R中用Spark知识的首要选择。文档会对示例，引用函数和其它更多相关资源保存持续更新。

* 博客

    要获取最新的重大`sparklyr`通知，你可以关注Rstudio博客。
...
现在，我们希望你已经准备好使用Spark和R分析真实数据和建模问题。这些内容会在接下来两章介绍。第3章会介绍数据分析，一个探查，清洗和转换数据的过程，达到发现有用信息的目的。第4章，建模，可以看做是数据分析的一部分。但是适合独立成章，以便真正阐述和利用Spark的建模功能。
...
之前的章节主要介绍Spark和R，可以让你快速熟悉和鼓励尝试基础的数据分析流程。但是，我们并没有介绍什么是数据分析，尤其是Spark中的数据分析。 我们介绍了本书中需要用到的工具，有助于你更多的关注学习而不是排查问题。

本章会介绍使用Spark和R进行数据分析的工具和概念。需要注意的是，这些工具和你只使用R的时候一样！这并非偶然。我们只是希望数据科学家能够屏蔽一些技术细节，而只使用自己熟知和喜欢的程序包，并且确实在Spark框架下可行。现在我们离这个目标还有点距离，但是并不远。所以，你会在本章学到一些广泛使用的R程序包和实际操作来进行数据分析——`dplyr`，`ggplot2`，`formulas`，`rmarkdown`等，这些程序包在Spark框架下依然有效。


第4章会关注创建统计模型来预测，估计和描述数据集。但是首先，我们从分析工作开始！

## 概述

数据分析项目的主要目标是理解数据到底要告诉我们什么，希望能够给出一个具体问题的答案。大部分数据分析项目遵循一些步骤，如图3-1所示。

如图所示，我们首先把数据引入主要分析流程。这个流程会通过各种数据转换，例如数据合并，完成*数据操作*。然后，我们会对数据*可视化*，以便看到数据的关系和趋势。要获得更深的理解，我们可以用一个或者多个统计模型集合数据。这会帮助我们找出相关模式在数据集上是否成立。最后，结果可以以公开的形式或者私密的形式和同事和相关人员沟通。
...
当处理小规模数据集，也就是内存允许的数据集时，我们可以按照R语言的步骤执行，不用使用Spark。但是，当数据及无法放入内存，或者计算太慢时，我们可以结合Spark优化这些步骤。但是如何优化呢？

对于数据分析师而言，理想的方法是让Spark做它擅长的事情。Spark是并行计算的引擎，可以在大数据集上工作，并提供SQL查询引擎和建模所用的各种库。你可以使用这些技术执行大部分与R相同的操作。这些操作，包括数据选取，转换，建模。

另外，Spark包含了一些执行特殊计算的工具，例如图分析，流式处理，还有其它工具。现在，我们跳过这些非表格数据集，留在后面的章节介绍。

你可以使用Spark进行数据*导入*，*操作*和*建模*。你也可以做一部分可视化工作，后边会介绍这一内容。最终目的是要使用R告诉Spark要做什么数据操作，并把结果返回给R。如图3-2所示，理想的流程可以*推动计算（pushes compute）* 到Spark集群中，并*收集结果（collects results）*返回给R。
...
![图3-3. R函数调用Spark函数](figures/3_3.png)

图中文字：

* Invoke，调用

对于更常见的数据任务，`sparklyr`提供了`dplyr`的强大支持。这就意味着，你可以使用R中已经熟知的`dplyr`操作，然后`sparklyr`和`dplyr`会把这些操作转换成Spark SQL语句。这些语句通常比SQL语句更加紧凑，易读（见图3-4）。所以，如果你对R和`dplyr`很熟悉，那就无需过多学习。这样介绍可能让人觉得有点虎头蛇尾。的确有点这种感觉。但是，可以只专注学习大规模计算技能也是很棒的体验。
...
为了边学边练，本章后边的代码使用简单的练习，可以在本地Spark集群上运行。这样，你可以在自己的电脑上重跑代码。确保`sparklyr`可以使用，第2章已经有过相关介绍。
...
使用Spark和R的时候，你需要使用不同的导入数据方法。通常，导入意味着R会读取文件，并加载到内存中。而使用Spark的时候，数据导入Spark，而不是R。在图3-5中，我们可以看到数据源是如何与Spark连接而不是和R连接的。
...
> 使用真实集群的时候，你应该只对小规模R数据表格使用`copy_to()`进行转换，大数据表格转换应该使用专门的数据转换工具。

现在Spark可以请求数据了，你可以轻松的进行数据转换。下一节会介绍如何在Spark中使用`dplyr`转换数据。

## 数据操作

数据操作利用数据转换理解数据。它通常指的是把“原始”数据转换成其它格式的数据，并试图利于后续数据分析的过程。
...
完成数据清洗，你还需要理解数据内容的基本情况。其它转换诸如聚合，可以帮助我们完成这个任务。例如，所有客户的平均余额结果会返回一个数据表。值是所有客户的平均值。当我们观察单个客户或群体客户的余额时，这些信息会提供一些背景知识。

尽可能使用R语言来编写数据转换的代码，这才是主要目的。这样可以省去单个任务中多个计算机技术之间来回切换的认知成本。这种情况下，最好使用`dplyr`，而不要编写Spark的SQL语句，完成数据转换。
...
虽然这些代码与非Spark环境下使用`dplyr`的代码相同，其内部还是做了很多运算。数据并*没有*导入到R中，相反，`dplyr`把这个任务变成SQL语句发送给Spark。`show_query()`命令允许查看由`sparklyr`和`dplyr`创建发送的SQL语句细节。我们也可以使用管道操作符（`%>%`），一种来自`magrittr`程序包的定制操作符，把计算结果当做下一个函数的第一个参数，进而让数据分析更加已读：
...
可以看到，`dplyr`比SQL语句简洁。但是请放心，使用`dplyr`的时候不需要关心和理解SQL。你的关注点一直在数据理解上，而不是给出SQL下的一系列数据转换语句。下面是另一个例子，对`cars`数据集按照`transmission`类型分组：
...
`dplyr`支持的大部分本地数据的转换操作也同样支持由Spark连接实现。这意味着，你可以首先关注学习`dplyr`，然后在Spark操作中复用。 由Hadley Wickham和Garrett Grolemund（O’Reilly）编写的《R语言数据科学》第5章是深入学习`dplyr`的良好资源。如果你已经熟练掌握`dplyr`，我们建议你花些时间，用不同的`dplyr`函数在`cars`数据表上做做实验。

有时，我们可能需要一些`dplyr`和`sparklyr`并不支持的操作。不要把数据加载到R中，而是找到一个Spark中的Hive函数完成所需的任务，下一节会介绍这个话题。

### 内置函数

Spark SQL基于Hive SQL的风格和函数而构建，并且可以使用`dplyr`调用所有的函数。这允许我们用任何一个Spark SQL函数来完成`dplyr`无法支持的数据操作。我们可以像R中的函数一样调用它们。`dplyr`可以把不曾认识的函数传递给查询引擎，为我们提供了使用函数的极大便利。

例如，`percentile()`函数返回组内一列的确切分位数。这个函数返回列名，以及一个分位数或一系列分位数。我们可以通过`dplyr`使用这个Spark SQL函数，如下所示:
...
要给`percentile()`传递多个值，我们可以调用另一个Hive函数`array()`。在这个例子中，`array()`类似于R中的`list()`函数。我们可以按照逗号分割，传递多个值。Spark的输出是一个数组变量，会以列表变量的形式导入到R中：
```{r}
...
我们在“Hive函数”的章节中罗列了所有的Hive函数。简单浏览一下，你就可以感受到Hive函数的强大运算能力。
...
可视化是帮助我们发现数据模式的重要工具。与从列表中读取异常值相比，画出一个图形在1000个观测值的数据集中识别异常值会更容易。

R擅长数据可视化。R语言的绘图能力可以通过许多关注分析的程序包来扩展。不幸的是，大部分绘图的R函数都需要数据加载在本地内存中，所以使用它并不能在Spark中使用远程数据表。
...
* Raw data，原始数据
* Transformed data，转换后数据
* Map。。。coordinates，把数据映射为坐标
* Draw plot，绘图

可视化方法本质上与数据操作一样：让Saprk做计算，然后收集结果绘制图形。如图3-8，准备数据的繁重工作，例如按照分组或者分箱聚合数据可以在Spark中完成，而规模小很多的数据集可以在R中操作。R中的绘图变成了非常基本的操作。例如，一个直方图的分箱可以在Spark中计算，而R语言不用绘制直方图，而是实现简单的柱形图绘制。R语言没有必要再计算一次分箱。
...
在这个例子中，`mtcars`原始数据自动转换成3个离散的聚合数值。然后，每个结果都映射到`x`和`y`轴。接着，画出图形。作为R用户，绘图的所有阶段都非常便捷的抽象出来。

在Spark中，有两个关键的步骤来执行“推动计算，收集数据”的理念。第一个是确保在Spark中完成转换操作。在后面的例子中，`group_by()`和`summarise()`会在Spark中运行。第二个是数据完成转换后返回给R。确保先转换数据后返回结果的顺序。如果`collect()`先运行，R会加载Spark中的全部数据集。根据数据集规模的不同，收集全部数据会拖慢你的系统，甚至宕机。
...
`dbplot`程序包为远程数据提供了有用的绘图函数。`dbplot`中用于转换数据的代码已经完成，以便可以对应到Spark中。它会使用这些结果，通过`ggplot2`程序包创建图形，其中数据转换和绘图都是由一个函数完成的。`dbplot_histogram()`函数可以让Spark计算分箱，并对每个分箱计数，产出一个`ggplot`对象，这个对象可以通过后续添加其它步骤进行优化。`dbplot_histogram()`也接受`binwidth`参数来控制用于计算分箱的数据范围：
...
* MPG distribution，MPG数据分布
* Histogram。。。galon，每加仑英里数直方图
* count，计数


直方图是分析单变量的好方法。要分析两个变量，我们经常会用到散点图或栅格图。散点图用来对比两个连续型变量的关系。例如，一个散点图会展示汽车重量和汽油消耗之间的关系。图3-11说明，汽车越重，汽油消耗越多，因为这些点几乎靠拢成一条线，从左上角下降到右下角。下面是生成图形的代码：
...
* Weight。。。MPG ，重量和MPG和重量
* A。。。galon，每加仑英里数与车重的散点图

然而，对于散点图，再多的“推动计算”也无助于解决这个问题，因为数据必须用单独的点绘制。

最好的选择是找到一种能代表x/y关系和集中度的绘图类型，且这种类型易于理解和实际绘图。栅格图可能是最好的答案。栅格图返回x/y位置的网格和给定聚集的结果，通常用带颜色的方形表示。

可以使用`dbplot_raster()`在Spark中创建类似散点图的绘图，而只获取（收集）远程数据集的一个子集：

```{r}
dbplot_raster(cars, mpg, wt, resolution = 16)
```

如图3-12所示，生成的图返回不大于$5 \times 5$的网格。这会把需要收集到R中的记录数限制为25。

![图3-12. 使用Spark绘制的点阵图](figures/3_12.png)

图中文字：

* Weight。。。MPG ，重量和MPG和重量
* A。。。galon，使用Spark计算的每加仑英里数与车重的点阵图


> **你也可以使用`dbplot`获取原始数据并用其它方式做可视化。要获取聚合结果，而不是图形，使用`db_compute_bins()`，`db_compute_count()`，`db_compute_raster()`和`db_compute_boxplot()`。**

虽然可视化是必不可少的，但你还可以使用统计模型补足数据分析，以获得对数据更深入的了解。下一节将介绍如何准备数据以便使用Spark进行建模。

## 建模

下面两章完全关注建模。本章不会过多地介绍建模的细节，而是介绍如何在做数据分析的时候与模型交互。

首先，一个分析项目经过很多数据转换和模型才能找出答案。这也是为什么图3-2中的第一个数据分析图展示了一个周期，包括：可视化，数据操作和建模。我们知道，不管是在R环境中还是使用Spark的过程中，你都不会止步于建模。
...
本节的例子使用非常小的数据集。在实际场景中，模型会使用大规模数据集。如果数据需要预先转换，会消耗Spark大量的进程开销。拟合模型之前，最好把所有转换的结果保存到一个Spark内存的新表中。
...
由于数据可以提炼出更多的理解，更多的问题也会随之提出来。这也是为什么我们希望通过数据操作，可视化和建模，来多次迭代。每次迭代应该增量的提供数据给我们传递的含义。我们会有一个时间点，来达到一个满意的理解水准。做到这一点，我们就可以分享分析的结果了。这就是下一章节的主题。
...
大部分输出可以通过R markdown的核心程序包生成，例如`knitr`和`rmarkdown`。你可以使用其它程序包扩展R Markdown。例如，本书就是用R Markdown编写的，基于`bookdown`程序包提供的扩展而完成。深度了解R Markdown的最优资源是官方书籍[^13]。
...
下面的例子展示了创建一个完全可复现的有关Spark处理大规模数据集的报告是多么的容易。介绍性文字，代码，以及最重要的代码输出都记录在最后的HTML文件中。你可以把下面的代码复制粘贴到一个文件中，并用*.Rmd*后缀保存起来，取一个喜欢的文件名字：
...
你可以轻松的分享这个报告，并不会消耗读者的Spark或R资源。它是一份自含的HTML文件，任何网络浏览器都可以打开。

从一个报告中提炼出各种观点并生成不同的输出格式也很常见。格式的转换也很容易，只需改变输出选项为`point_presentation`，`pdf_document`，`word_document`或类似选项。你甚至可以从一份报告中生成多个输出格式：
...
本章对R和Spark的数据分析进行了扎实的介绍。提出的许多技术看起来与只使用R而不使用Spark非常相似。这并非虎头蛇尾，而是让已经熟悉R的用户简单的过渡到Spark的正确设计。对于不熟悉R的用户，本章还非常简要介绍R中一些最流行的（有用的）程序包。

现在应该很清楚，R和Spark是一个强大的组合，一个大型计算平台，以及一个异常强大的R程序包生态系统，可以成为理想的分析平台。

在使用R和Spark分析时，请记住将计算推送到Spark，并在R中收集结果。这种范式应该建立了一个成功的途径，以各种输出格式分享结果，实现数据操作、可视化和沟通。
...
我们首先结合Spark的背景以及贯穿全章的数据集，介绍建模。然后我们会展示一个有监督的学习流程，包括探索性数据分析，特征工程，和模型构建。接着，我们会使用非结构化的文本数据继续介绍无监督的主题建模示例。切记，我们的目标是介绍大规模数据集上执行数据科学任务的各种技术，而不是指导严谨一致的分析。Spark中还有许多其它的模型并不会在本章中介绍。，但是在本章的末尾，你会看到一些不错的工具，可以试试用自己的模型做实验。
...
可以看到，Spark提供了大量的算法和特征变换函数。这里我们只涉及具有代表性的一些功能。完整 的预测模型概念讲解已经超出了本书的范畴，所以我们推荐阅读Hadley Wickham和Garrett Grolemund G (O’Reilly)的*《R语言数据科学》*，以及*Feature Engineering and Selection: A Practical Approach for Predictive Models*[^14]。本章中的一些例子和可视化也是从这些材料中引用过来（有时候会原封不动的引用）。
...
* Distribution of age，年龄分布
* Age。。。dataset，OKcupid数据集中的年龄直方图
* Age，年龄
* count，计数
...
```
在图4-5中，我们看到缩放后的`age`变量取值位于零附近。下面我们讨论其它类型的转换，但是在做特征工程的时候，你可能会希望对所有模型中用到的数值变量进行归一化。
...
* Distribution。。。age，缩放后`age`变量的分布
* Scaled。。。dataset，OKcupid数据集中的缩放后`age`直方图
...
* Distribution。。。Length，缩放后文章长度变量的分布
* Essay。。。dataset，OKcupid数据集中缩放后的文章长度直方图
...
一旦我们有了数据集，我们就可以开始构建一些模型。在我们开始之前，需要制定一个计划来调整和验证备选模型。在建模项目中，我们经常尝试不同类型的模型和方法来进行拟合，看看哪一个表现最好。因为我们处理的是一个二分类问题，可用的指标包括准确度，精确度，灵敏度，以及接受者操作特性曲线（ROC AUC）下的面积，以及其它度量方法。要优化的指标取决于特定业务问题，但是在本次练习中，我们将重点放在ROC AUC上。
...
* ROC curve，ROC曲线
* Receiver。。。model，逻辑回归模型的受试者操作特性曲线
...
> Spark只为广义线性模型（包括线性模型和逻辑回归）提供了评估方法。对于其它算法，你可以使用评估函数（例如，数据框的`ml_binary_classification_evaluator()` ，或者是自己的度量算法。
...
* Coefficient。。。intervals，95%置信区间的系数估计
...
```
然后，我们可以按主题查看单词概率来可视化输出。在图4-10和图4-11中，我们显示了1次迭代和100次迭代的结果。下面是生成图4-10的代码；要生成图4-11，你需要在运行`ml_lda()`时设置`max_iter = 100`，但要注意在单机上这可能需要长时间——这是一个大计算问题，而一个合适Spark集群可以很容易地解决。
...
正如我们在介绍本章时提到的，本章重点是预测建模。Spark可以助力大规模数据科学项目，也可以产品化数据科学工作流程，使之可以自动化，也就是我们熟知的机器学习。第5章介绍工具，让预测模型，以及整个训练流程，融合到自动化环境中，可以持续的运行，或者导出到web应用程序、移动应用程序和其它环境中。
...
在本章中，我们将深入学习Spark管道操作，它们可以推动第4章中介绍的功能。例如，当通过R中的公式接口调用`MLlib`函数时，例如`ml_logistic_regression(cars, am ~ .)`，一个*管道*就在背后构建完成了。管道允许使用高级数据处理和建模流程。此外，管道还允许将其*部署*到生产系统，网络应用程序、移动应用程序等,促进跨数据科学和工程团队的协作。

本章也是鼓励使用本地计算机作为Spark集群的最后一章。你离真正的集群计算，以及数据科学或机器学习只有一章之遥，它们可以扩展到最苛刻的计算问题上。

## 概述

管道的组件是称为*转换器*和*评估器*的对象，它们统称为*管道阶段*。转换器可将转换操作应用于数据框并返回另一个数据框；生成的数据框通常包含原始数据框及其追加的新列。另一方面，评估器可以用来创建一个提供训练数据的转换器。可以用下面的例子来说明这种关系：一个“居中和缩放“评估器可以学习一些数据的平均值和标准差，并将统计信息存储在最后的转换器对象中；然后，可以使用这个转换器来归一化它所训练的数据以及任何新的、但看不见的数据。
...
<standard_scaler_7f6d46f452a1>
 ...
 with_std: TRUE
```
现在我们可以创建一些数据（我们已经知道平均值和标准差），然后使用`ml_fit()`函数拟合缩放模型：
```{r}
...
> 在Spark ML中，许多算法和特征变换都要求输入为矢量列。函数`ft_vector_assembler()`可以执行此任务。你还可以使用该函数初始化要在管道中使用的转换器。

我们看到，平均值和标准差非常接近0和1，这正是我们期望的。借助`ml_transform()`函数，我们可以使用转换器来转换数据框：
```{r}
scaler_model %>%
ml_transform(df) %>%
glimpse()
...
在`sparklyr`中，有两种构建管道的方法，二者都使用`ml_pipeline()`函数。
...
> 基于Spark ML的设计，即使管道只包含转换器，它也始终看成是评估器对象。这意味着，如果有一个只有转换器的管道，你仍然需要对其调用`ml_fit()`，以获得一个转换器。本例中的“拟合”过程实际上不会修改任何转换器。

## 用例

既然你已经了解了ML管道的基本概念，那么让我们把它们用到上一章中的预测建模问题中。在上一章中，我们试图通过查看人们的画像来预测他们目前是否有工作。我们从带有相关列的`okc_train`数据框开始。
...
前三个阶段通过`ft_string_indexer()`为`sex`、`drinks`和`drugs`字符型列建立了数值索引。这对于接下来的`ft_one_hot_encoder_estimator()`是必要的，因为它需要数值列输入。当我们所有的预测变量都是数值类型时（回想一下，`age`已经是数值类型），我们可以使用`ft_vector_assembler()`创建特征向量，它将其所有输入拼接成一个列向量。然后我们可以使用`ft_standard_scaler()`归一化特征列的所有元素（包括类别型变量的独热编码的0/1值），最后通过`ml_logistic_regression()`使用逻辑回归。

在原型设计期间，你可能会把数据框传递给`ft_`和`ml_`函数并检查转换后的数据框，*迫切地*在数据的一小部分上执行这些转换。即时的反馈方便快速迭代想法。当你获得所需的处理步骤时，你可以将它们打包成一个管道。例如，你可以执行以下操作：
...
找到数据集的合适转换后，可以用`ml_pipeline(sc)`替换数据框输入，结果是一个可以应用到任何具有合适结构的数据框的管道。在下一节中，我们将看到，管道如何使我们更容易的测试不同的模型配置。

### 超参调试

回到前面创建的管道，我们可以使用`ml_cross_validator()`来执行在上一章中介绍的交叉验证工作流，并轻松测试不同的超参组合。在这个例子中，我们测试逻辑回归中变量的中心化和各种正则化手段是否可以改善预测。我们定义交叉验证器，代码如下：
...
`estimator`参数只是我们想要调整的评估器，在本例中，它是我们定义的管道。我们通过`estimator_param_maps`参数给我们感兴趣的超参提供数值，该参数采用一个嵌套的命名列表。第一级的名称对应UID，它是与每个我们要调试的管道阶段对象关联的唯一标识符（如果只提供了部分UID，`sparklyr`将尝试将其与管道阶段匹配）。而第二级名称对应于每个阶段的参数。在开始的代码片段中，我们指定要测试以下内容：
...
我们希望这些信息可以生成$2 × 2 × 2 = 8$种超参组合，我们可以通过打印`cv`对象来确认：
...
既然我们看到了工作中的管道API，让我们更加正式的介绍它们在不同的问题背景下是如何运行的：

## 操作模式

截止到目前，你可能已经注意到了，管道阶段函数，例如`ft_string_indexer()`和`ml_logistic_regression()`会根据传入的第一个变量返回不同类型的对象。表5-1给出了完整的模式。
...
这些函数是使用S3实现的，S3是R提供的最流行的面向对象编程范例。就我们的目的而言，只需知道`ml_`或`ft_`函数的行为是由提供的第一个参数的类决定的就可以了。这可以让我们在不引入额外函数名的情况下提供很多功能。现在我们总结一下这些函数的行为：

* 如果提供了Spark连接，则函数返回一个转换器或评估器对象，它们可以直接通过`ml_fit()`或`ml_transform()`调用，也可以包含在管道中。
* 如果提供了管道，函数将返回一个追加有阶段的管道对象。
* 如果将数据框提供给特征变换函数（带有前缀`ft_`的函数），或者不带公式的ML算法，则该函数将实例化管道阶段对象，必要时会完成拟合（如果阶段是评估器），然后转换数据框。
* 如果给支持公式接口的ML算法提供了数据框和公式，那么`sparklyr`会在后台构建管道模型，并返回包含其它元数据信息的ML模型对象。

公式接口方法是我们在第4章中学到的，这就是我们推荐给Spark新手的入门方法，因为它的语法类似于现有的R程序包，并且抽象掉了一些Spark ML特性。然而，为了充分利用Spark ML的强大功能并利用管道进行工作流组织和交互性，学习ML管道的API还是很值得的。
...
管道最强大的功能之一是，它们可以序列化到磁盘，并且可以与其它Spark API（如Python和Scala）完全可交互。这意味着你可以在使用不同语言的Spark用户之间轻松地共享成果，其中可能包括其它数据科学家、数据工程师和部署工程师。要保存管道模型，请调用`ml_save()`并提供路径：
...
> 为（使用公式接口创建的）`sparklyr` ML模型调用`ml_save()`时，会保存关联的管道模型，但不会保存任何`sparklyr`相关的元数据，例如索引标签。换句话说，保存一个`sparklyr` `ml_model`对象以及后续的加载将生成一个管道模型对象，就像是通过ML管道API创建的一样。这个行为需要与其它编程语言一起使用管道。

在我们继续讨论如何在生产环境中运行管道之前，确保你断开了Spark：
...
再次强调一下我们刚刚介绍的内容：通过在ML管道框架内协作，我们减少了数据科学团队中不同角色之间的摩擦。特别的，我们减少了从建模到部署的时间。

在许多情况下，一个数据科学项目并不仅仅是以一个带有见解和建议的幻灯片结束。相反，手头的业务问题可能需要实时的按计划或按需求给新的数据点打分。例如，一家银行可能希望每晚评估其抵押贷款组合风险，或就信用卡申请提供即时决策。将模型转换为其它人可以使用的服务的过程通常称为*部署*或*产品化*。历史上，构建模型的分析人员和部署模型的工程人员之间存在巨大的鸿沟。前者可能使用R并开发了大量打分机制的文档，以便后者可以在C++或Java中重新实现模型。这种做法在一些组织中很可能需要几个月的时间。虽然这种做法并不盛行。但其实在Spark ML工作流中几乎都不必要。

前面提到的每晚投资组合风险和信贷应用评分示例代表了两种ML部署模式，即*批处理*和*实时处理*。简单的说，批处理是指同时处理许多记录，而且只要执行时间合理（通常以分钟到小时为单位），执行时间不重要。而实时处理意味着一次给一条或几条记录打分，但延迟是至关重要的（在小于1秒的范围内）。现在让我们来看看如何把我们的`OKCupid`管道模型应用到“生产”中。


### 批打分

对于批打分和实时打分两种方法，我们将以超文本传输协议（Hypertext Transfer Protocol，HTTP）API的形式将模型发布为web服务，这是软件通信的主要桥梁。通过提供API，其它服务或最终用户可以在不了解R或Spark的情况下利用我们的模型。R程序包`plumber`通过注释我们的预测函数让我们能够很容易做到这一点。
...
`callr`包支持在单独的R进程中运行R代码。这不是必须的，但我们通过它在后台启动web服务。`httr`包允许我们使用R的网络API。
...
这个结果告诉我们，这个人很可能不是失业，也就是在业。我们现在可以通过停止`callr`服务来终止`plumber`服务：
...
我们将模型保存为MLeap bundle格式的方法与使用Spark ML 管道API保存模型的方法非常相似；唯一的附加参数是`sample_input`，它是一个Spark数据框，带有给新数据打分的模式：
...
如果我们对这个操作计时，可以看到服务现在可以在几十毫秒内返回预测结果。
...
在本章中，我们讨论了Spark管道，这是第4章中介绍的建模函数背后的引擎。你学习了如何通过将数据处理和建模算法组织到管道中来整理预测建模工作流。你知道了管道通过共享与语言无关的序列化格式促进了多语言数据科学和工程团队成员之间的协作。也就是说，你可以从R导出Spark管道，让其它人使用Python或Scala将管道重新加载到Spark中，这使得他们可以在不改变语言选择的情况下进行合作。

你还学习了如何使用`mleap`部署管道。它是一个Java运行时，提供了另一条产品化Spark模型的方法，即你可以导出管道并将其集成到支持Java的环境中，而不要求目标环境支持Spark或R。

你可能注意到一些算法，特别是无监督的学习算法，速度很慢。甚至对于可以加载到内存中的`OKCupid`数据集也是如此。如果我们能够访问一个合适的Spark集群，我们就可以用更多的时间建模，用更少的时间等待！不仅如此，我们还可以使用集群资源运行更广泛的超参数优化作业并处理大型数据集。为了达到这个目的，第6章介绍了什么是计算集群，并解释了值得你考虑的各种选项，比如建立自己的集群或按需使用云集群。
...
前几章集中讨论了在一个计算实例上,即你的个人计算机上，使用Spark。在本章中，我们将介绍在多个计算实例（也称为计算集群）上运行Spark的技术。本章和后续章节将介绍使用适用于计算集群的概念；但是，并不需要使用计算集群来继续学习，因此你仍然可以使用个人计算机。值得一提的是，虽然前面的章节主要关注一个计算实例，但你也可以在计算集群中使用介绍的所有数据分析和建模技术，而无需更改任何代码。

如果你的组织中已经有了Spark集群，可以考虑跳到第7章，它会教你如何连接到现有的集群。否则，如果没有集群或正在考虑对现有基础设施改进，本章将介绍目前可用的集群趋势、管理者和供应商。

## 概述

集群计算有三大趋势值得讨论：内部部署、云计算和Kubernetes。根据时间刻画趋势将有助于我们了解它们是如何实现的，它们是什么，以及它们未来可能是什么。为了说明这一点，图6-1使用来自谷歌趋势的数据绘制了它们随时间变化的趋势图。

对于内部集群，你或你组织中的某个人购买了用于集群计算的物理计算机。集群中的计算机可以由*现成*硬件组成，即下单购买通常在商店货架上就可以找到的计算机；也可以由*高性能*硬件组成，即计算供应商提供了高度定制的计算硬件，并针对高性能网络连接、功耗等进行了优化。

![图6-1. 内部部署（主机），云计算和Kubernetes的谷歌趋势](figures/6_1.png)

图中文字：

* Cluster。。。trend，聚类计算趋势
* search。。。kubernetes，内部部署（主机），云计算和Kubernetes的搜索流行趋势
* popularity，流行趋势


在购买成百上千个计算实例的时候，将它们放在熟悉的常见机箱中是没有意义的；相反，尽可能高效地将它们堆叠在一起以最小化使用空间是有意义的。这组高效堆叠的计算实例称为*机架*。当集群增长到数千台计算机之后，你需要托管数百个计算设备机架；在这种规模下，你还需要大量的物理空间来托管这些机架。

提供计算实例机架的建筑通常称为*数据中心*。在数据中心的维度上，你还需要找到提高建筑物利用率的方法，特别是冷却系统、电源、网络连接等。由于这是一项耗时的工作，一些组织以开放计算项目计划（open Compute Project initiative）的名义联合起来，开放其基础设施的源代码。该计划为任何人免费提供一组数据中心蓝图。

没有什么可以阻止你构建自己的数据中心。事实上，许多组织都已经开始了这条道路。例如，亚马逊最初是一家在线书店，但经过几年的发展，它的销售远远不止于书籍。随着在线商店的增长，其数据中心的规模也在增长。2002年，亚马逊考虑把数据中心的服务器出租给公众。两年后，亚马逊网络服务（Amazon Web Services，AWS）推出，让任何人都可以按需租用该公司数据中心的服务器，这意味着你需要购买、配置、维护或拆除自己的集群；或者，你也可以直接从AWS租下来。

这种按需计算模型就是我们今天熟知的云计算。在云中，你使用的集群不归你所有，也不在你的物理建筑中；而是被其它人拥有和管理的数据中心。今天，这个领域有许多云供应商，包括AWS、Databricks、谷歌、微软、Qubole和许多其它公司。大多数云计算平台通过web应用程序或命令行提供用户界面来请求和管理资源。

虽然多年来在云中处理数据的好处是显而易见的，但选择一个云供应商却产生了意想不到的副作用，即把组织锁定到一个特定供应商，使其很难在供应商之间切换或返回到本地集群。2014年谷歌发布的Kubernetes是一个开源系统，用于跨多个主机管理容器化应用程序。实际上，它使跨多个云供应商和本地部署变得更容易。

总而言之，我们已经看到了从本地到云计算的转变，以及最近的Kubernetes。这些技术通常被简单地描述为私有云、公共云，以及启用混合云的某一编排服务。本章将带你了解Spark和R环境中的每一种集群计算趋势。

## 本地化

如概述部分所述，内部部署集群代表一组由组织中的工作人员采购和管理的计算实例。这些集群可以高度定制和控制；但是，它们也可能产生较高的初始费用和维护成本。

在使用本地Spark集群时，你应该考虑两个概念：

* 集群管理器
    与操作系统（如Windows或macOS）允许你在同一台计算机上运行多个应用程序类似，集群管理器也允许多个应用程序在同一个集群中运行。在使用内部部署集群时，你需要自己选择一个集群管理器。
* Spark发行版
    虽然你可以从Apache Spark站点安装Spark，但是许多组织与能够为Apache Spark提供支持和增强服务的公司合作，我们通常称之为Spark发行版。
...
在Spark单机中，Spark使用自己作为自己的集群管理器，这允许你无需在集群中安装其它软件而使用Spark。如果你计划使用集群仅运行Spark应用程序，这可能很有用；如果该集群不只用于Spark，那么像YARN、Mesos或Kubernetes这样的通用集群管理器会更适合。Spark单机文档包含有关配置、启动、监视和启用高可用性的详细信息，如图6-2所示。

由于Spark单机包含在第2章介绍的Spark安装中，你可以使用Spark安装来初始化自己机器上的本地Spark 单机集群。实际工作中，你可能希望启动不同机器上的工作节点，但为了简单起见，我们提供了在单个机器上启动单机集群的代码。
...
然后，我们可以使用主URL初始化单个工作节点。你也可以使用类似的方法，即多次运行代码，在不同的机器上初始化多个工作节点：
...
Spark单机中只注册了一个工作节点。点击这个工作节点的链接，可以查看这个节点的细节，例如内存和处理器，如图6-4所示。
...
Hadoop YARN，或者简称为YARN，是Hadoop项目的资源管理器。它最初是在Hadoop项目中开发的，但在Hadoop 2中被重构为单独的项目。正如我们在第1章中提到的，Spark是为了加快Hadoop上的计算速度而构建的，因此在Hadoop集群上安装Spark很常见。

YARN的一个优点是它可能已经安装在许多支持Hadoop的现有集群上，这意味着你可以轻松地使用Spark以及许多现有的Hadoop集群，而不需要对现有集群的基础设施进行任何重大改变。Spark部署在YARN集群中也非常常见，因为许多集群最初是Hadoop集群，最终也会升级支持Spark。

你可以通过两种模式提交YARN应用：YARN客户端和YARN集群。在YARN集群模式下，驱动程序（可能）正在远程运行，而在YARN客户端模式下，驱动程序在本地运行。这两种模式都支持，我们将在第7章中进一步介绍它们。

YARN提供了一个资源管理用户界面，可用于访问日志、监视可用资源、终止应用程序等。从R连接到Spark后，你可以用YARN管理正在运行的应用程序，如图6-5所示。
...
与YARN一样，Mesos支持执行许多集群框架，包括Spark。然而，Mesos的一个优势是它允许像Spark这样的集群框架实现定制的任务调度器。调度器是集群中协调应用程序，分配执行时间和资源的组件。Spark使用粗粒度的调度器，它根据应用程序的持续时间调度资源；但是，其它框架可能使用Mesos的细粒度调度器，它可以通过在较短的时间区间内调度任务来提高集群的整体效率，从而允许任务之间共享资源。

Mesos提供了一个web界面来管理正在运行的应用程序、资源等。从R连接到Spark后，你的应用程序像Mesos中运行的任何其它应用程序一样完成注册。图6-6显示了从R到Spark的成功连接。
...
正如前一节所介绍的，你可以在本地集群中使用集群管理器；但是，许多组织（包括但不限于Apache Spark）选择与提供附加管理软件、服务和资源的公司合作，以帮助管理其集群中的应用程序。一些本地集群供应商包括*Cloudera*、*Hortonworks*和*MapR*。下面我们会简要介绍它们。
...
Cloudera集群使用包。包是包含程序文件和元数据的二进制发行版。Spark恰好作为一个包安装在Cloudera上。介绍如何配置Cloudera集群超出了本书的范围，但是可以在Cloudera.com和Cloudera博客上的《Introducing `sparklyr`, an R Interface for Apache Spark》下找到资源和文档。

Cloudera提供了Cloudera管理器网络接口来管理资源、服务、包、诊断等。图6-7显示了运行在Cloudera管理器中的Spark包，你可以稍后使用它连接R。
...
Hortonworks是一家总部位于加州圣克拉拉的大数据软件公司。公司开发、支持并提供一整套完全开源的软件，旨在管理从物联网（IoT）到高级分析和机器学习的数据和处理。Hortonworks相信它是一家连接云和数据中心的数据管理公司。

Hortonworks与微软合作，改善微软Windows对Hadoop和Spark的支持。这一点曾是区别于Cloudera的一个不同点；然而，自从两家公司于2019年1月合并以来，Hortonworks和Cloudera的对比在今天就不那么重要了。尽管已经合并，对Cloudera和Hortonworks的Spark发行版的独立支持仍然可用。在Hortonworks下配置Spark的其它资源可以在Hortonworks.com上找到。

MapR也是一家总部位于加利福尼亚州圣克拉拉的商业软件公司。MapR提供从单个计算机集群访问各种数据源的权限，包括大数据工作负载，如Apache Hadoop和Apache Spark，分布式文件系统，多模式数据库管理系统和事件流处理，实时分析与操作应用的结合。它的技术运行在商业硬件和公有云计算服务上。

## 云端

如果你既没有本地集群也没有备用计算机可重用，那么从云集群开始可能会非常方便，因为它允许你在几分钟内访问正确的集群。本节简要介绍了一些主要的云基础设施供应商，并提供了一些资源，帮助你在选择使用云供应商时上手。

在云服务中，计算实例的计费时间参考Spark集群的运行时间。当集群启动时开始计费，当集群停止时停止计费。此成本需要乘以为集群保留的实例数。因此，例如，如果一个云供应商每小时从每个计算实例收费1.00美元，而你启动一个3节点的集群，使用一小时10分钟，那么你会收到$1.00美元 \times 2小时 \times 3节点=6.00美元$的账单。 一些云供应商按分钟收费，但你可以选择他们按小时计算收费的方案。

请注意，虽然小型集群的计算成本可能很低，但不经意地让集群一直运行可能会导致高额的帐单费用。因此，值得花额外的时间检查两次集群是否在不再需要时就可以停止。这也是每天使用集群时监测成本的优质实践，同时确保每天的账单符合你的预期。

根据以往的经验，在处理大型项目时，你还应该计划提前请求计算资源。但是在发出正式请求之前，各种云供应商不允许启动包含数百台计算机的集群。虽然这可能很麻烦，但这也是帮助你控制成本的一种方法。

由于集群的大小很灵活，从小型集群开始并按需扩展计算资源是一个很好的实践。即使你事先知道你会需要一个很大规模的集群，从小规模开始也提供了一个以较低成本解决问题的机会，因为你的数据分析不太可能在第一次尝试大规模运行时毫无问题。经验法则是，以指数方式增长实例；如果需要在八节点集群上运行计算，需要从一个节点和整个数据集的八分之一开始，然后是两个节点和四分之一数据集，然后是四个节点和一半数据集，最后是八个节点和整个数据集。当你变得更有经验时，会对如何解决问题以及所需集群的大小有一个很好的了解，并且可以跳过中间步骤。但对于新手来说，这是一个很好的实践。

你还可以使用云供应商获取计算资源，然后自己安装上一节中介绍的内部部署发行版；例如，你可以在亚马逊的弹性计算云（Amazon Elastic Compute cloud，Amazon EC2）上运行Cloudera发行版。此模型可以避免获取同位置的硬件，但它仍然允许你密切管理和定制集群。本书只简单介绍了云供应商提供的完全托管的Spark服务。但是，你通常可以在网上轻松找到有关如何在云中安装本地发行版的说明。
...
使用R和亚马逊EMR的具体说明发表在Amazon的大数据博客中，名为《Running `sparklyr` on Amazon EMR》。这篇博文介绍了`sparklyr`的启动和通过其配置亚马逊集群的说明。例如，它推荐你使用亚马逊命令行界面（Amazon Command Line
Interface）启动三个节点的集群，代码如下：
...
然后，你可以看到集群启动，并最终在AWS站点中运行，如图6-8所示。

接着，你可以导航至Master Public DNS，并在端口8787下找到RStudio，例如`ec2-12-34-567-890.us-west-1.compute.amazonaws.com:8787`，然后使用用户名`hadoop`和密码`password`登陆进入。

![图6-8. 启动亚马逊EMR集群](figures/6_8.png)

你也可以使用网络界面启动亚马逊EMR集群。同一篇介绍性博文包含专门为亚马逊EMR设计的其它详细信息和步骤。
...
实例| CPU| 内存| 存储| EC2成本| EMR成本
------|-----|----|-----|-------|------|-----
c1.medium |2 |1.7GB |350GB |0.148美元/小时 |0.030美元/小时
m3.2xlarge |8 |30GB |160GB |0.616美元/小时 |0.140美元/小时
i2.8xlarge |32 |244GB |6400GB |7.502美元/小时|0.270美元/小时

> 我们仅介绍了截至2019年亚马逊和后续云供应商提供额可用计算实例的一个子集；但是 需要注意，硬件（CPU速度、硬盘驱动器速度等）因供应商和位置而异。因此，你不能使用这些硬件表作为准确的价格比较。准确的比较需要运行特定的工作负载并考虑计算实例成本之外的其它方面。
...
其它资源可以在Databricks工程博客文章《Using `sparklyr` in Databricks》和`sparklyr`的Databricks文档下获得。
...
谷歌提供了谷歌云Dataproc，作为谷歌云平台（GCP）上提供的云端托管Spark和Hadoop的服务。Dataproc利用许多GCP技术，如谷歌计算引擎和谷歌云存储，来提供运行主流数据处理框架（如Apache Hadoop和Apache Spark）的完全托管集群。
...
有各种教程（cloud.google.com/dataproc/docs/tutorials），包括配置RStudio和`sparklyr`的综合教程，可以参考。

你可以在cloud.google.com/dataproc/pricing上找到最新的定价信息。在表6-4中，注意成本是在计算引擎和dataproc高级版之间分摊的。

表6-4. 谷歌云Dataproc定价信息

实例 |CPU|内存 |计算引擎| Dataproc高级版
------|------|------|------|------
n1-standard-1 |1 |3.75GB |0.0475美元/小时 |0.010美元/小时
n1-standard-8 |8 |30GB |0.3800美元/小时 |0.080美元/小时
n1-standard-64 |64| 244GB |3.0400美元/小时 |0.640美元/小时

### IBM

IBM云计算是一套面向业务的云计算服务。IBM云包括通过公共、私有和混合云交付模型提供的基础设施即服务（IaaS）的云、软件即服务的云（SaaS）和平台即服务的云（PaaS），以及组成这些云的组件。

在IBM云服务中，打开Watson Studio并创建一个数据科学项目，在项目设置下添加一个Spark集群，然后在Launch IDE菜单上启动RStudio。请注意，截至写书的时候，提供的`sparklyr`版本不是CRAN中提供的最新版本，因为`sparklyr`完成了修改以便在IBM云下运行。在任何情况下，请把IBM的文档作为权威参考，在IBM云上运行R和Spark，特别是如何正确的升级`sparklyr`。图6-11展示了启动Spark集群的IBM的云站点。
...
实例 |CPU |内存| 存储 |成本
------|------|-------|-----|------
C1.1c1x25 |1 |1GB |25GB |0.033美元/小时
C1.4x4x25 |4 |4GB |25GB |0.133美元/小时
C1.32x32x25 |32 |25GB |25GB |0.962美元/小时

### 微软

微软Azure是由微软创建的云计算服务，实现通过微软管理的数据中心全球网络来构建、测试、部署和管理应用程序和服务。它提供SaaS、PaaS和IaaS，并支持不同的编程语言、工具和框架，包括微软特定的和第三方软件和系统。

通过Azure站点，Azure HDInsight服务为定制化Spark集群提供支持。通过选择ML Services集群类型，可以轻松创建支持Spark和RStudio的HDInsight集群。注意，提供的`sparklyr`版本可能不是CRAN中的最新版本，因为默认的程序包仓库似乎是使用微软R应用程序网络（MRAN）来初始化的，而不是直接从CRAN初始化的。图6-12显示了使用R启动Spark集群的Azure站点。
...
实例 |CPU |内存 |总成本
-----|-----|------|--------
D1 v2 |1 |3.5GB |0.074美元/小时
D4 v2 |8 |28GB |0.59美元/小时
G5 |64 |448GB |9.298美元/小时
...
Kubernetes是一个开源的容器编排系统，用于容器化应用程序的自动部署、扩展和管理，最初由谷歌设计，现在由云原生计算基金会（Cloud Native Computing Foundation，CNCF）维护。Kubernetes最初基于Docker，而和Mesos一样，它现在也基于Linux Cgroups。

Kubernetes可以执行许多集群应用和框架。你可以通过使用具有特定资源和库的容器镜像进行高度定制。这可以让Kubernetes集群用于数据分析之外的许多不同目的，进而帮助组织轻松地管理他们的计算资源。使用自定义镜像需要权衡进一步的配置开销问题，但又使Kubernetes集群非常灵活。然而，这种灵活性在许多组织中都证明有助于轻松管理集群资源。正如之前概述中所指出的，Kubernetes正在成为一个非常流行的集群框架。

所有主要的云供应商都支持Kubernetes。它们都提供了关于如何启动、管理和拆除Kubernetes集群的大量文档；图6-14展示了创建Kubernetes集群时的GCP控制台。你可以在任何Kubernetes集群上部署Spark，并且可以使用R来完成连接、分析、建模等操作。

![图6-14. 使用谷歌云为Spark和R创建Kubernetes集群](figures/6_14.png)

你可以在kubernetes.io上了解更多信息，并通过Spark.apache.org阅读《Running Spark on Kubernetes》。
...
通过第1章的学习，你知道RStudio是R的一个著名的免费桌面开发环境；因此，很可能大家正在使用RStudio Desktop完成本书中的示例。但是，你可能不知道RStudio也可以作为web服务在Spark集群中运行。这个版本的RStudio称为RStudio Server。你可以在图6-15中看到正在运行的RStudio Server。与Spark UI在集群中运行的方式相同，你可以在集群中安装RStudio Server，然后连接到RStudio Server，并按照RStudio Desktop完全相同的方式使用RStudio，同时支持针对Spark集群运行代码。如图6-15所示，RStudio Server不管看起来，还是感觉起来都像RStudio Desktop，但是它支持通过所在的集群环境中来高效的运行命令。

![图6-15. 运行在AWS的RStudio Server专业版](figures/6_15.png)

如果你熟悉R的话，Shiny服务是从R构建交互式web应用程序的一个非常流行的工具。我们建议你直接在Spark集群中安装Shiny。

RStudio和Shiny服务器是免费开源的产品；但是，RStudio还提供RStudio Server、RStudio Server Pro、Shiny Server Pro和RStudio Connect等专业产品。你可以在集群内安装这些产品以支持其它R工作流程。`sparklyr`不需要任何额外的工具，但它们带来了值得考虑的生产力显著提高。你可以在rstudio.com/products/上了解更多相关信息。


### Jupyter

Juyter项目为开发开源软件、公开标准和跨几十种编程语言的交互式计算服务而生。Jupyter笔记本支持多种编程语言，包括R。你可以使用R内核保证`sparklyr`与Jupyter笔记本一起使用。图6-16展示了`sparklyr`在本地Jupyter笔记本中运行的情况。

![图6-16. 运行`sparklyr`的Jupyter笔记本](figures/6_16.png)

### Livy

Apache Livy是Apache中的一个孵化项目，支持通过web界面远程使用Spark集群。直接连接到Spark集群是最理想的。但是，有时直接连接到Spark集群并不可行。当面临这些限制时，你可以考虑在集群中安装Livy并对其进行适当的安全保护，以便能够通过web协议进行远程使用。不过，请注意，在`sparklyr`中使用Livy会带来明显的性能开销。
...
你可以导航至本地http://localhost:8998上的Livy进程。第7章会具体介绍如何通过Livy连接。连接成功后，你可以导航至Livy的web应用，如图6-17所示。
...
本章介绍了内部部署和云计算的历史和权衡，以及Kubernetes这一提供跨内部部署或多个云供应商之间灵活切换的有前途的框架。本章还引入了集群管理器（Spark单机、YARN、Mesos和Kubernetes），作为运行Spark集群应用所需的软件。本章还简要介绍本地集群供应商，如Cloudera、Hortonworks和MapR，以及主要的Spark云供应商：亚马逊、Databricks、IBM、谷歌、微软和Qubole。

虽然本章为理解当前集群计算趋势、工具和服务供应商在处理大规模数据科学项目上提供了坚实的基础，但它并没有提供一个全面的框架来帮助你决定选择哪种集群技术。相反，你可以将本章作为概述和起点来寻找其它资源，以帮助找到最适合组织需要的集群。
...
第6章介绍了主要的集群计算趋势、集群管理器、发行版和云服务供应商，以帮助你选择最适合需要的Spark集群。相比之下，本章会介绍Spark集群内部组件以及如何连接到特定Spark集群。

阅读本章时，无需执行每一行代码，否则会非常困难，因为你需要准备不同的Spark环境。相反，如果你已经有了一个Spark集群，或者上一章让你有足够的动力去注册一个按需的集群，那么现在是学习如何连接集群的时候了。本章帮助你连接到已经选择的集群。如果还没有选择集群，我们建议你学习这些概念，然后回来运行相关代码。

此外，本章还提供了各种连接故障排除技术。虽然我们不希望你使用它们，但还是保证你可以运用这些技术解决连接问题。
...
Spark集群所有连接的体系结构由三种类型的计算实例组成：*驱动节点*、*工作节点*和*集群管理器*。集群管理器是一种允许Spark在集群中执行的服务。这部分内容第之前的“管理器”中有详细说明。工作节点（也称为执行器）在分区数据上执行计算任务，并将中间结果传递给其它工作节点或返回给驱动节点。驱动节点的任务是将工作委托给工作节点，同时还负责聚合它们的结果和控制计算流。在大多数情况下，聚合发生在工作节点中；然而，即使在节点聚合数据之后，驱动节点通常也需要收集工作节点的结果。因此，驱动节点通常至少（通常更多）配备一些计算资源（内存、CPU、本地存储等），可以作为工作节点。

严格地说，驱动节点和工作节点只是分配了具有特定角色的机器的名称，而驱动节点中的实际计算则根据*SparkContext*执行。Spark Context是Spark功能的主要入口，因为它的任务是调度任务、管理存储、跟踪执行状态、指定访问配置的设置、取消作业等。在工作节点中，实际计算是在spark执行器下执行的，Spark执行器是一个Spark组件，其任务是针对特定的数据分区执行子任务。图7-1说明了这个概念，其中驱动节点通过集群管理器编排一个工作节点的工作。
...
* Worker，工作节点
* Spark executor， Spark执行器

如果你的组织中已经有一个Spark集群，那么你应该向集群管理员请求指向该集群的连接信息，仔细阅读它们的使用策略，并遵循它们的建议。因为集群可以在许多用户之间共享，所以你需要确保只请求所需的计算资源。我们将在第9章中介绍如何请求资源。系统管理员将指定它是本地集群还是云集群、使用的集群管理器、支持的连接和支持的工具。你可以使用这些信息直接跳转到本地、单机、YARN、Mesos、Livy或Kubernetes，根据这些信息可以选择适合需求的方案。
...
* web browser，网络浏览器
* Edge，边缘节点
* Secure shell Server，Secure shell服务器
* RStudi server，RStudio服务器
...
如果你的集群供应商或集群管理员已经提供了*SPARK_HOME*，那么下面的代码应该返回路径而不是空字符串：
...
当你以本地模式连接到Spark时，Spark会像Spark Context和单个执行器一样，启动一个运行大多数集群组件的进程。这对于在大型计算集群上运行Spark、脱机工作、故障排查或测试代码非常理想。图7-3描述了Spark的本地连接。
...
注意，本地模式下既没有集群管理器，也没有工作进程，因为在本地模式下，所有工作都在驱动应用程序中运行。另外值得注意的是，`sparklyr`通过`spark-submit`启动Spark Context。`spark-submit`是每个Spark安装程序中都提供的一个脚本，允许用户向Spark提交自定义应用程序。如果你好奇的话，第13章会解释`sparklyr`中发生的提交此应用程序并从R正确连接的内部过程。

要执行本地连接，我们可以使用前面章节中熟悉的代码：
...
连接到Spark单机集群需要集群管理器主节点实例的位置。就像之前在“单机”部分介绍的，你可以在集群管理器的网络界面中找到该实例。你可以通过查找以spark://开头的URL来找到此位置。

单机模式下的连接从`sparklyr`开始，它会启动`spark-submit`，然后提交`sparklyr`应用程序并创建Spark Context。该Context从运行在给定主节点地址下的Spark单机实例中请求执行器。图7-4说明了此过程，这与图7-1中的总体连接架构非常相似，但附加一些单机集群和`sparklyr`特有的细节。
...
* Driver，驱动节点
* Cluster manager，集群管理器
* Spark Standlone，Spark单机
* Worker，工作节点
* Spark executor（S）， Spark执行器
...
当你使用YARN客户端模式连接时，驱动实例运行R，`sparklyr`和Spark Context。这些操作需要YARN工作节点来运行Spark执行器，如图7-5所示。
...
当使用客户端模式运行YARN时，集群管理器会在后台执行你希望的操作：它从集群分配资源并把它们分配给Spark应用程序。Spark Context会为你管理这些应用。在之前的“YARN”部分中有一点需要注意，Spark Context位于运行R代码的同一台机器中；这与集群模式下运行YARN有所不同。

### YARN集群

在集群模式下运行YARN与在客户端模式下运行YARN的主要区别在于，在集群模式下，不要求驱动节点是启动R和`sparklyr`的节点。相反，驱动节点仍然是专门指定的，通常与运行R的边缘节点不同。当边缘节点有太多并发用户、缺少计算资源或诸如RStudio或Jupyter的工具需要区别于其它集群资源管理工具进行独立管理时，考虑使用集群模式可能会有帮助。

图7-6展示了在集群模式下运行时不同组件是如何分离的。注意，仍然有一条连接客户端和集群管理器的线路，因为我们仍然需要从集群管理器分配资源；在分配资源之后，客户端直接与驱动节点通信，驱动节点与工作节点通信。从图7-6中，你可能认为集群模式看起来比客户端模式复杂得多。这种认识是正确的。因此，如果可能的话，最好避免集群模式，因为它有额外的配置开销。
...
与其它需要在集群中使用边缘节点的连接方法不同，Livy提供了一个web API，使Spark集群可以从集群外部访问，并且不需要在客户端中安装Spark。在通过web API连接之后，*Livy服务*通过从集群管理器请求资源并像往常一样分发工作来启动Spark Context。图7-7展示了一个Livy连接。注意，客户端通过web API远程连接到驱动节点。
...
要在本地机器上使用Livy，你可以像之前在“Livy”中描述的那样，安装运行Livy服务并进行连接，代码如下：
...
`MESOS_NATIVE_JAVA_LIBRARY`需要经过系统管理员的设置，或者在本地机器上运行Mesos的时候手动设置。例如，在MacOS中，你可以用终端安装和初始化，然后通过手动设置Mesos库，并使用`spark_connect()`进行连接：
...
Kubernetes并不像Mesos或者YARN支持客户端模式。相反，连接模型类似于YARN集群，其中的驱动节点由Kubernetes指定，如图7-9所示。
...
如果你的计算机已经完成使用Kubernetes集群的配置，可以使用下面的命令找出`apiserver-host`和`apiserver-port`：
...
当你使用云服务供应商时，有不同的连接方式。例如，从Databricks连接需要下面的方法：
...
如果需要帮助，参考你的云服务供应商文档和支持渠道。

## 批量模式

大多数情况下，你可以交互使用`sparklyr`；也就是说，你可以显式地与`spark_connect()`连接，然后执行命令来分析和建模大规模数据。但是，你也可以通过调度使用`sparklyr`的spark作业来自动化进程。Spark不提供调度数据处理任务的工具；相反，你要使用其它工作流管理工具。这对于转换数据、准备模型和隔夜数据打分，或者其它系统使用Spark非常有用。
...
你可以通过给`spark_submit()`传递额外的命令行参数来定制你的脚本，并使用R的`commandArgs()`读取出来。

## 工具

使用Jupyter和RStudio等工具连接到Spark集群时，你可以运行本章中介绍的相同连接参数。但是，由于许多云供应商使用web代理来保护Spark的web界面，要使用`spark_web()`或RStudio Connections窗口扩展，你需要正确配置`sparklyr.web.spark`，然后通过`config`参数将其传递给`spark_config()`。
...
通常，我们只会连接Spark一次，而且只有一次。但是，你也可以通过连接到不同的集群或指定`app_name`参数来多次打开Spark连接。这有助于在提交到集群之前比较Spark版本或验证你的分析结果。以下示例打开了与Spark 1.6.3、2.3.0和Spark单机的连接：
...
最后，我们介绍一下故障排除技术：日志记录、Spark Submit和Windows。如果不确定从何处开始，请在使用Windows系统时从Windows部分开始，然后是日志记录，最后是Spark Submit。当运行`spark_connect()`失败并显示错误消息时，这些技术非常有用。
...
然后，使用故障排除中正确的`master`参数替换`"local"`计算圆周率例子如下：
...
如果前面的消息没有显示，你需要调查为什么你的Spark集群没有正确配置。这超出了本书的范围。首先，重新运行圆周率例子，但是要删除`stderr = FALSE`；这会将错误消息打印到控制台，然后你可以使用控制台的消息来调查问题可能的原因。当使用云供应商或Spark发行版时，你可以联系他们的支持团队帮助进一步解决此问题；另外，Stack Overflow也是一个很好的调查问题的起点。如果你确实看到消息，这意味着你的Spark集群已正确配置，但是不知何故R无法使用Spark，因此你还需要详细地解决此问题，我们接下来会解释。
...
如果第一个连接步骤失败，意味着集群无法接受应用程序。这通常意味着没有足够的资源，或者存在权限限制。
...
如果第二个连接步骤失败，通常意味着R和驱动节点之间存在连接问题。例如，你可以尝试使用其它连接端口。

### Windows

在大多数情况下，Windows连接和从Linux和macOS连接一样简单。但是，有几个常见的连接问题需要注意：

* 防火墙和防病毒软件可能会阻止端口连接。`sparklyr`使用的默认端口是8880；请仔细检查此端口是否未被阻止。
* 长路径名可能会导致问题，特别是对于像Windows7这样的旧Windows系统。使用这些系统时，请尝试与所有文件夹一起安装的Spark连接，最多使用8个字符，名称中不要有空格。
...
在第8章中，我们会介绍如何使用Spark读写来自各种数据源和格式的数据，这可以让你在数据分析时更加灵活的添加新的数据源。过去需要几天，几周，甚至几个月完成的事情，现在你可以通过拥抱数据湖在数小时内完成。
...
此外，我们还将讨论如何扩展Spark的能力，使其能够处理无法直接访问的数据，并提出一些提高数据读写性能建议。读取大型数据集通常需要调试Spark集群配置，但这是第9章的主题。

## 概述

在第1章中，你学习到除了大数据和大计算之外，还可以使用Spark来提高数据任务的速度、多样性和准确性。虽然你可以将本章的知识用于任何需要加载和存储数据的任务中，但在处理各种数据源时，本章的内容更加有趣。为了理解原因，我们首先快速看一下，当今数据在许多组织中是如何处理的。

多年来，人们经常把大型数据集存储在关系*数据库*中，这是Edgar F.Codd于1970年首次提出的一种系统[^21]。你可以将数据库视为相互关联的表的集合，其中每个表都经过精心设计以保存特定的数据类型，以及与其它表的关系。大多数关系数据库系统使用*结构化查询语言（Structured Query Language，SQL）*来查询和维护数据库。数据库在今天仍然被广泛使用，这是有充分理由的：它们可靠且一致地存储数据；事实上，你的银行很可能会将帐户余额存储在数据库中，这是一种很好的实践。

[^21]: Codd EF (1970). “A relational model of data for large shared data banks.”

然而，数据库也可以用来存储来自其它应用程序和系统的信息。例如，你的银行还可能存储其它银行生成的数据，例如传入的支票。要做到这一点，需要从外部系统中提取外部数据，转换成适合当前数据库的数据，最后加载到数据库。这称为*提取*、*转换*和*加载*（ETL），是一种将数据从一个或多个源复制到目标系统的通用过程。该目标系统以不同于源的方式表示数据。ETL过程从20世纪70年代开始流行。

除了数据库之外，数据通常还加载到*数据仓库*中，数据仓库是一个用于报告和数据分析的系统。数据通常以提高数据分析速度的格式存储和索引，但这种格式通常不适合建模或运行自定义的分布式代码。挑战在于，更改数据库和数据仓库通常是一个漫长而微妙的过程，因为需要重新建立数据索引，并且需要小心地将来自多个数据源的数据转换为可被多个数据源共享的单个表。
...
为了支持各种各样的数据源，Spark需要能够读写几种不同的文件格式（CSV、JSON、Parquet和其它格式），并访问存储在几个文件系统（HDFS、S3、DBFS等）中的数据，并且可能与其它存储系统（数据库、数据仓库等）进行交互。我们会讨论所有这些内容，但首先，我们会介绍如何使用Spark读取、写入和复制数据。
...
在分析数据时，通常将多个文件加载到单个数据对象中。在R中，我们通常使用循环或函数式编程指令来实现这一点。这是因为R必须单独加载每个文件并传递到R进程中。让我们在文件夹中创建几个CSV文件，并先用R进行读取：
...
“文件夹即表”的思想也可以在其它开源技术中找到。Hive表的后台工作原理是一样的。当你查询一个Hive表时，映射会在同一文件夹中的多个文件上完成。文件夹的名称通常与用户可见的表的名称匹配。
...
你也可以把列的说明设置为一个显式包含列类型的向量。这个向量的值按照与字段名匹配的方式命名：
...
然后，在使用`spark_read_csv()`读取时，可以通过`spec_with_r`给`columns`传递参数，以匹配原始文件的名称和类型。这有助于提高性能，因为Spark不再需要确定列类型。
...
在Spark中，格式错误的记录会在读取过程中导致错误，特别是对于非字符型字段。为了防止这种错误，我们可以使用一个文件说明，将字段作为字符型导入，然后使用`dplyr`将字段强制为所需的类型。

本小节回顾了如何能够更快地读取文件且失败更少，从而使我们能够更快地开始分析。另一种加速分析的方法是将更少的数据加载到Spark内存中，我们会在下一节中介绍这个话题。

### 内存

默认情况下，当使用Spark和R环境读取数据时，数据被复制到Spark的分布式内存中，使得数据分析和其它操作非常快速。有些情况下，例如当数据太大时，加载所有数据可能不可行，甚至不必要。对于这些情况，Spark只需“映射”文件，而无需将数据复制到内存中。

映射在Spark中创建了一种虚拟表。这意味着，当查询该表时，Spark需要从文件中读取数据。任何后续读取都将执行相同的操作。实际上，Spark变成了高速的数据传递方法。这种方法的优点是几乎不需要提前“读取”文件；映射速度非常快。缺点是运行实际提取数据的查询需要更长的时间。
...
这个方法有一些很好的用例，其中一个用例是不需要表的所有列。以包含许多列的非常大的文件为例。假设这不是你第一次与这些数据交互，那么你可能知道分析需要哪些列。当你知道需要哪些列时，可以使用`memory=FALSE`读取文件，然后使用`dplyr`选择所需的列。然后，可以使用`compute()`函数将生成的`dplyr`变量缓存到内存中。这会告诉Spark查询文件，抽取所选字段，并只将数据复制到内存中。读取结果是，读取时间相对较短的内存中的表：
...
Spark 1.6要求对列名进行清洗，所以默认情况下R是这样做的。在某些情况下，你可能希望保持原始名称的完整性，或者在使用Spark 2.0或更高版本。为此，可以把`sparklyr.sanitize.column.names`选项设置为`FALSE`：
...
有些项目要求将Spark中生成的新数据写回远程的数据源。例如，数据可以是由Spark模型返回的新预测值。Spark作业会处理大量的预测生成任务，预测值需要存储。本节重点介绍如何使用Spark将数据从Spark传递到外部目标数据地址。
...
应该尽一切努力将Spark连接到最终目标位置。这样，读、处理和写就发生在同一个Spark进程中。如图8-3所示，更好的方法是使用Spark来读取、处理和写入目标位置。这种方法能够扩展到Spark集群能力所及的范围，并防止R成为瓶颈。

![图8-3. 写入大型数据集时Spark的正确方式](figures/8_3.png)
...
回到我们的场景，Spark中有一个需要保存的大型数据集。当Spark和目标位置在同一个集群中时，复制结果是没有问题的；数据传输是在同一集群的内存和磁盘之间进行的，或者通过高速连接高效的完成。但如果目标位置不在Spark集群内怎么办？有两个选项，需要根据数据大小和网络速度选择一个：

* Spark传输
    在这种情况下，Spark连接到远程目标位置并复制新数据。如果这是在同一个数据中心或云供应商内完成的，那么数据传输速度可能足够快，保证Spark可以直接写入数据。
* 外部传输和其它方式
    Spark可以将结果写入磁盘并通过第三方应用进行传输。Spark将结果作为文件写入，然后一个单独的作业完成文件复制。你可以使用单独的进程将数据传输到目标位置。

需要意识到Spark、R和任何其它技术都是工具。没有工具可以做任何事情，你也不应该期望它能做任何事。接下来我们将介绍如何将数据复制到Spark或收集内存无法加载的大型数据集，这些数据集可用于跨集群传输或帮助初始化分布式数据集。 

## 复制数据

前几章使用`copy_to()`作为复制数据到Spark的便捷助手；但是，你只能使用`copy_to()`来传输已经加载到内存中的数据集。这些数据集往往比要复制到Spark中的数据集要小得多。
...
或者，你也可以通过一个`collect()`的回调任务来收集不适合内存的数据集。回调只是一个R函数，它会在每个Spark分区上调用。然后你可以将数据集写入磁盘或通过网络推送到其它集群。即使收集数据集的驱动节点内存小于3GB，也可以使用以下代码。也就是说，正如第3章所介绍的，你应该避免将大型数据集收集到一台计算机中，因为这会造成一个明显的性能瓶颈。为了简洁起见，我们只收集前100万行；如果时间允许，可以放心移除`head(10^6)`：
...
以下章节将从著名的CSV文件格式开始，介绍每种文件格式的特殊注意事项以及一些主流文件格式的优缺点。

### CSV

CSV格式可能是目前使用的最常见的文件类型。它是一个使用给定的字符（通常是逗号）进行分隔的文本文件。读取CSV文件应该非常简单；但是，值得一提的是，有两种技术可以帮助你处理与优质格式的CSV文件不完全兼容的CSV。Spark提供了以下解决问题的模式：
...
Spark提供了一个问题跟踪列，该列在默认情况下是隐藏的。要启用它，使用`add _corrupt_record`操作`columns`列表。你可以把这个方法与`PERMISSIVE`模式相结合。所有行都会导入，无效条目将返回`NA`，并在`_corrupt_record`中跟踪此问题：
...
在大多数系统中，以csv的形式读取和存储数据非常常见，而且是广泛支持的。对于表格式数据集，它仍然是一个主流的选项，但是对于包含嵌套结构和非表格式数据的数据集，JSON通常是首选。
...
另一种方法是使用`install.packages("sparklyr.nested")`从CRAN安装`sparkly.nested`，然后使用`sdf_unnest()`拍平嵌套的数据：
...
虽然JSON和csv使用起来非常简单且通用，但它们并没有针对性能进行优化；而其它格式（如ORC、AVRO和Parquet）则进行了优化。
...
为了说明这一点，图8-4描绘了使用`bench`程序包运行100万行写入速度的基准结果；在决定哪种格式最适合你的需要时，可以在有意义的数据集上使用自己的基准：
...
连接时需要将程序包加载到Spark中。要加载包，Spark需要它的位置，它可能在集群内、共享文件中或互联网上。
...
此外，R社区还开发了一些扩展来加载其它文件格式，例如`sparklyr.nested`用于嵌套数据，`spark.sas7bdat`用于从SAS读取数据，`sparkavro`用于以AVRO格式读取数据，`sparkwarc`用于读取WARC文件，这些扩展使用了第10章中介绍的扩展机制。第11章介绍了使用R程序包加载其它文件格式的技术，第13章介绍了使用Java库做了进一步补充。但首先，让我们探讨如何从几个不同的文件系统中检索和存储文件。

## 文件系统

Spark默认采用当前运行它的文件系统。在YARN管理的集群中，默认的文件系统是HDFS。*/home/user/file.csv*的示例路径将从集群的HDFS文件夹（而不是Linux文件夹）读取。操作系统的文件系统将用于其它部署，如独立部署和`sparklyr`的本地部署。

文件系统协议可以在读取或写入时更改。你可以通过`sparklyr`函数的`path`参数来实现这一点。例如，*file://home/user/file.csv*的完整路径强制使用本地操作系统的文件系统。

还有许多其它文件系统协议，例如Databricks文件系统的*\_dbfs://\_*、Amazon的S3服务的*\_s3a://\_*、Microsoft Azure存储的*\_wasb://\_*和Google存储的*\_gs://\_*。

Spark并没有直接为所有这些组件提供支持；相反，它们是根据需要配置的。例如，访问“s3a”协议需要向`sparklyr.connect.packages`配置添加一个包，而连接服务和确定正确的请求凭据可能需要使用`AWS_ACCESS_KEY_ID`和`AWS_SECRET_ACCESS_KEY`环境变量。
...
访问其它文件协议需要加载不同的包，但在某些情况下，提供Spark环境的供应商可能会为你加载包。请参考供应商的文件，以了解是否存在这种情况。


## 存储系统

数据湖和Spark通常是同时存在的，可以选择访问数据库和数据仓库等存储系统。用适当的示例演示所有不同的存储系统非常耗时，因此我们将介绍一些常用的存储系统。

首先，Apache *Hive*是一个数据仓库软件，它使用SQL方便地读取、写入和管理分布式存储中的大型数据集。事实上，Spark将Hive的组件直接嵌入到它的源中。同时安装Spark和Hive是很常见的，因此我们首先介绍Hive，然后介绍Cassandra，再介绍JDBC连接。

### Hive

在YARN管理的集群中，Spark提供了与Apache Hive更深入的集成。

打开Spark连接后，可以方便地访问Hive表。在SQL语句中引用表，使用`DBI`访问该表的数据：
...
必须重申，没有数据导入到R中，`tbl()`函数只创建一个引用。然后，你可以在`tbl()`命令之后添加更多`dplyr`谓词作为管道操作：
...
在`dplyr`中，可以使用`in_schema()`函数。这个函数用在`tbl()`调用中：
...
Apache *Cassandra*是一个免费的、开源的、分布式的、宽列存储的NoSQL数据库管理系统，旨在跨许多商业服务器处理大量数据。虽然除了Cassandra之外还有许多其它数据库系统，但快速查看Spark中Cassandra的使用方法，可以让你深入了解如何使用其它数据库和存储系统，如Solr、Redshift、Delta Lake等。
...
用`spark_read_jdbc()`和`spark_write_jdbc()`连接到远程数据库非常容易；你只需要能够访问适当的JDBC驱动程序。这有时是无需提起的，而有时则存在风险。为了简单起见，我们可以简单考虑一下如何实现到远程MySQL数据库的连接。

首先，你需要从MySQL的开发人员网站下载合适的JDBC驱动程序，并把这个额外的驱动程序指定为`sparklyr.shell.driver-class-path`连接选项。由于JDBC驱动程序是基于Java的，所以代码包含在JAR（Java存档）文件中。一旦你使用合适的驱动程序连接到Spark，就可以使用*jdbc://*协议访问特定的驱动程序和数据库。除非你愿意自己下载和配置MySQL，否则请跳过执行以下语句：
...
我们还介绍了在Spark中读取、写入和复制数据的最佳实践。然后我们又回来研究数据湖的组成部分：文件格式和文件系统。前者表示数据的存储方式，后者表示数据的存储位置。然后，你学习了如何处理需要额外Spark包的文件格式和存储系统，以及一些跨文件格式的性能权衡问题，并学习了在Spark中使用存储系统（数据库和数据仓库）所需的概念。
...
在前面的章节中，我们假设Spark中的计算是高效的。虽然这个假设在一些情况下是成立的，但通常需要对Spark内部运行的操作有一些了解才能调试配置，从而使计算高效运行。本章介绍Spark如何在大型数据集上的数据做计算，并提供有关优化操作的详细信息。

例如，在本章中，你将学习如何请求更多的计算节点以及增加内存。如果你还记得第2章的内容，本地实例中的内存默认为只有2GB。你将了解Spark如何通过分区、重洗和缓存来统一计算。就像前几章提到的，这是介绍Spark内部机制的最后一章；在完成本章之后，我们相信你会具备使用Spark所必需的中级技能。

在第10-12章中，我们会探索一些令人兴奋的技术，可以处理具体的建模、可扩展性和计算问题。首先，我们必须了解Spark如何执行内部计算，我们可以控制哪些部分，以及为什么要控制。

## 概述

Spark通过在多台计算机上配置、分区、运行、重洗、缓存和序列化数据、任务和资源来执行分布式计算：

* 配置工作需要集群管理器提供资源：全部机器，内存等。
* 分区会在不同的机器之间分割数据。分区可以是隐式的，也可以是显式的。
* 运行意味着在每个分区上运行任意转换。
* 重洗会把数据重新分发到正确的机器。
* 缓存会跨不同的计算周期将数据保存在内存中。
* 序列化转换要通过网络发送给其它工作节点或返回数据到驱动节点。

要说明每一个概念，我们需要创建一个无序整数的三个分区，并使用`arrange()`进行排序：
...
图9-1显示了这个排序作业在理论上是如何跨一个机器集群工作的。首先，Spark会将集群配置为使用三个工作节点。在本例中，数字1到9被划分为三个存储实例。因为数据已经分区，所以每个工作节点都加载这个隐式分区；例如，4、9和1加载在第一个工作节点中。然后，将任务分发给每个工作节点，对每个工作节点中的每个数据分区应用转换；此任务用`f(x)`表示。在本例中，`f(x)`在分区内执行排序操作。由于Spark是通用的，所以分区上的执行可以根据需要进行简化或变得复杂。

然后将结果打乱传入正确的机器，以完成整个数据集的排序操作，从而完成一个阶段。*阶段*是一组操作，Spark可以执行这些操作，而无需在机器之间对数据重洗。在集群对数据进行排序之后，可以选择将排序后的结果*缓存*在内存中，以避免多次重新运行这个计算任务。

最后，通过连接集群计算机的网络将一小部分结果*序列化*，并将结果返回到驱动节点以打印此排序示例的预览。

注意，虽然图9-1描述了排序操作，但类似的方法也适用于筛选或连接数据集以及大规模数据的分析和建模。Spark提供了执行自定义分区、自定义重洗等的支持，但这些较低级别的操作大多没有在`sparklyr`中开放。相反，`sparklyr`通过数据分析工具，如`dplyr`或`DBI`，进行建模和使用许多扩展提供的更高级别的命令使这些操作可用。对于那些可能需要实现低级别操作的少数情况，你始终可以通过`sparklyr`扩展使用Spark的Scala API或运行自定义的分布式R代码。
...
在我们的示例中，理解给定操作，例如示例中排序任务Spark计算图的最佳方法是打开Spark的web接口中SQL选项卡上的*最后完成查询*。图9-2显示了排序操作的最终计算图，其中包含以下操作：

* `WholeStageCodegen`
    此模块说明，它包含的操作用于生成可高效转换至字节码的计算机代码。将操作转换为字节码通常会带来很小的成本，但这是值得付出的代价，因为在Spark中可以更快地执行操作。通常，你可以忽略这个模块，将焦点放在它包含的操作上。
* `InMemoryTableScan`
    此模块说明，原始数据集存储在内存中并一次性按行转换。
* `Exchange`
...
*事件时间线*是有关Spark跨每个阶段的计算周期开销的一个汇总。理想情况下，你希望看到这个时间线主要由CPU使用量组成，因为其它任务可以被视为开销。你还希望看到Spark使用所有可用集群节点上的全部CPU。

选择当前作业中的第一个阶段并展开事件时间线，它应该类似于图9-4。注意，我们显式地请求了三个分区，它们在这个可视化中由三个道表示。

![图9-4. Spark事件时间线](figures/9_4.png)

由于我们的机器配备了四个CPU，我们可以使用`sdf_repartition()`显式地重新分区数据，进一步实现并行化计算。结果如图9-5所示：
...
图9-5展示了四个执行通道，其中在执行器计算上花费的时间最多。这表明这个特定的操作正在更好地利用我们的计算资源。当使用集群时，请求集群上更多计算节点应该会缩短计算时间。与此相反，对于那些展示花了大量时间进行重洗的时间线，请求更多的计算节点可能不会缩短时间，实际上还可能会使所有操作都变慢。我们并没有一套具体的规则用来优化一个阶段；但是，根据在多个操作中获得时间线的经验，你会深入了解如何正确优化Spark操作。
...
> 建议为驱动节点请求远多于每个工作节点上可用的内存量。在大多数情况下，你需要为每个工作节点请求一个内核。
...
使用Spark单机和Mesos集群管理器时，默认情况下会分配所有可用内存和内核；因此，不需要更改其它配置，除非你希望限制资源以便允许多个用户共享集群。在这种情况下，你可以使用`total-executor-cores`来限制请求的执行器总数。Spark单机和Mesos管理下的Spark指南提供了有关共享集群的其它信息。
...
在集群模式下使用YARN时，可以使用`sparklyr.shell.driver-cores`配置在驱动节点中请求的总内核数。《Spark on YARN》指南提供了额外的配置设置，可以帮助你。
...
* *提交*
     这些设置会在`sparklyr`通过`spark-submit`提交到Spark时使用；有些设置取决于所使用的集群管理器。
* *runtime*
     这些设置会在创建Spark进程时配置Spark。它们独立于集群管理器，并且关联具体的Spark。
* *`sparklyr`*
     这些设置会配置`sparklyr`行为。这些设置独立于集群管理器，并且专属于R。
...
你可以将表9-1中列出的参数与`spark_connect()`一起使用。它们会配置高级设置，定义连接方法、Spark的安装路径和要使用的Spark版本。

表9-1. 连接Spark所用的参数

名称 |取值
-----|------
`master` |要连接的Spark集群URL。使用`"local"`连接至一个使用`spark_install()`安装的本地Spark实例。
...
可以通过在`config`参数中指定列表来配置其它设置。现在让我们看看这些设置有什么。
...
`sparklyr.shell.packages`| 包含在驱动节点和执行节点类路径上的JAR的Maven坐标逗号分隔列表。将搜索本地Maven repo，然后搜索Maven Central和`sparklyr.shell.repositories`提供的任何其它远程仓库。坐标的格式应为`groupId:artifactId:version`。
`sparklyr.shell.exclude-packages`|`groupId:artifactId`的逗号分割列表，排除解决`sparklyr.shell.packages`提供的依赖时使用，以避免依赖冲突。
`sparklyr.shell.repositories`|其它远程仓库的逗号分割列表，以便使用`sparklyr.shell.packages`搜索Maven坐标。
...
`sparklyr.shell.keytab` |包含刚才指定的principal的keytab的文件的完整路径。此keytab将通过安全分布式缓存复制到运行应用主机的节点，以便定期更新登录凭证和委派令牌。

通常，任何`spark-submit`设置都通过`sparklyr.shell.X`来配置，其中X是`spark-submit`参数不带`--`前缀的名称。

### runtime设置

正如提到的，一些Spark设置会配置进程的runtime。runtime设置是提交设置的超集。因为即使不能更改设置，检索当前配置通常也是有帮助的。

要列出当前Spark进程中的Spark设置，可以运行以下命令：
...
但是，Spark中还有更多的配置，如《Spark Configuration》指南中所述。这超出了本书的范围，所以，如果可能的话，可以花点时间来确定那些可能对你的特定用例有关的配置。


### `sparklyr`设置

除了Spark设置，还有一些专门的`sparklyr`设置。通常在调试Spark时不使用这些设置；而在从R排除Spark故障时很有用。例如，可以使用`sparklyr.log.console = TRUE`将Spark日志输出到R控制台；这在排除故障时是可取的，但在其它情况下有点烦人。下面介绍了如何列出设置（结果如表9-5所示）：
...
`sparklyr.apply.schema.infer` |当`spark_apply()`给定列类型时，用于推断模式的行数。
...
`sparklyr.collect.batch` |使用批收集时要收集的总行数，默认为100,000。
...
`sparklyr.connect.jars` |在给Spark提交应用时，包含的其它的JAR。
...
`sparklyr.gateway.routing` |`sparklyr`网关服务是否要发送到其它进程？可以在Kubernetes中关闭。
...
`sparklyr.verbose.sanitize` |是否在清洗列和其它对象的时候使用详细日志记录？
...
`sparklyr.yarn.cluster.start.timeout` |放弃等待YARN集群应用完成注册之前的总秒数。

## 分区

如第1章介绍，MapReduce和Spark的设计目的是对存储在许多机器上的数据进行计算。每个计算实例上用于计算的数据子集称为分区。
...
你可以使用`sdf_num_partitions()`，来查看计算所需的分区个数：
...
各种数据函数，比如`spark_read_csv()`，已经支持`repartition`参数来正确地请求Spark对数据进行重新分区。例如，我们可以创建一个由10个数字组成，分区为10的序列，如下所示：
...
回忆一下第1章，Spark通过使用内存而不是磁盘来存储数据，从而比以前的产品更快。这就是所谓的Spark*弹性分布式数据集（resilient distributed dataset，RDD）*。RDD将相同数据的副本分布在多台机器上，这样，即使一台机器出现故障，其它机器还可以完成任务，因此，这就是术语“弹性”的含义。弹性在分布式系统中很重要，因为，虽然计算通常在一台机器上完成，但当数千台机器运行时，发生故障的可能性还是要高得多。当任务失败时，最好能够容错，以避免失去其它机器上的工作。RDD通过跟踪数据沿袭信息来实现这一点，以便在失败时自动重建丢失的数据。

在`sparklyr`中，可以使用`tbl_cache()`和`tbl_uncache()`控制何时从内存加载或卸载RDD。

大多数获取Spark数据框的`sparklyr`操作都会将结果缓存在内存中。例如，运行`spark_read_parquet()`或`copy_to()`会提供已经缓存在内存中的Spark数据框。Spark数据框这个对象可用于大多数`sparklyr`函数，包括使用`dplyr`或机器学习进行数据分析：
...
你可以通过保存CSV、Parquet和其它文件格式显式记录检查点。或者，让Spark在`sparklyr`中使用`sdf_checkpoint()`进行检查点记录，如下所示：
...
作为调试执行的一部分，你可以考虑通过创建具有不同于Spark默认值的Spark连接来调整分配给用户、执行和存储的内存大小：
...
例如，如果你希望使用Spark将大量数据存储在内存中，以便快速筛选和检索子集，可以让Spark使用很少的执行或用户内存。因此，为了最大化存储内存，可以按照以下方式调整Spark：
...
但是，请注意，如果需要和可能的话，Spark会从存储内存中借出一些给执行内存，反之亦然。因此，在实践中，很少需要调整内存设置。

## 重洗

重洗是一种跨机器重新分配数据的操作；它的成本很高，因此应该尽量减少这种操作。通过查看事件时间线，你可以很容易地确定是否在重洗上花费了大量的时间。通过重新组织数据分析问题或适时告诉Spark，可以减少重洗。

例如，当连接大小差异很大的多个数据框时，这就相关了。也就是说，一个集合比另一个集合小几个数量级。可以考虑使用`sdf_broadcast()`将数据框标记为足够小，以便在broadcast的join中使用，这意味着它将一个较小的数据框推送到每个工作节点，以减少对较大数据框的重洗。以下是`sdf_broadcast()`的一个示例：
...
在调试Spark时，调整序列化并不常见；但是，值得一提的是，其它序列化模块（如Kryo序列化器）可以提供基于默认的Java序列化器的性能改进。你可以通过以下操作打开`sparklyr`中的Kryo序列化器：
...
本章介绍了Spark内部机制和具体配置的诸多内容，以帮助你加速计算，并启用高计算负载。它可以为理解常见配置的瓶颈和指导提供基础。然而，调试Spark是一个广泛的话题，需要更多的章节来详尽介绍。因此，在对Spark的性能和可扩展性进行故障排除、搜索网络资源和咨询在线社区时，通常还需要对你的特定环境进行调试。
...
> 我总是尽量多认识一些人。你永远不知道你需要哪个人。
> —Tyrion Lannister

在第9章中，你学到了Spark如何通过允许用户配置集群资源、隐式或显式地分区数据、跨分布式计算节点执行命令、在必要时跨节点重洗数据、缓存数据以提高性能，以及通过网络高效地序列化来处理大规模数据。你还学习了如何配置连接、提交作业和运行应用程序时使用不同的Spark设置，以及针对本章介绍的R和R扩展的特定设置。

第3，4章和第8章为读取和理解大多数数据集提供了基础。然而，所提供的功能仅限定在Spark的内置特性和表格数据集上。本章突破表格数据，探讨如何通过图处理技术来分析和建模包含互联对象的网络，分析基因组学数据集，为深度学习准备数据，分析地理数据集，以及在大规模数据集上使用高级建模库（如H2O和XGBoost）。

前几章中介绍的所有内容应该能够满足大部分大规模计算需求。但是，对于那些仍然缺乏功能支持的少数用例，下面的章节提供了一些工具来扩展Spark。例如，你可以通过自定义R转换、自定义Scala代码或Spark中最新的执行模式，从而能够分析实时数据集。然而，在重新发明轮子之前，让我们来看看Spark中的一些扩展。

## 概述

在第1章中，我们把R社区呈现为一个充满活力的群体，其中的每个人在许多方面相互协作。例如，通过创建可以从CRAN安装的R程序包来推动开放科学的发展。以类似的，但规模要小得多的方式，R社区提供了一些扩展，扩大了Spark和R最初支持的功能。而Spark本身也支持创建Spark扩展。事实上，许多R扩展都使用Spark的扩展。

人们经常创建扩展，所以本章的内容在某个时间点后就过时了。此外，我们甚至可能不知道许多Spark和R扩展。然而，我们至少可以通过查看CRAN中`sparklyr`的“reverse imports”来跟踪CRAN中可用的扩展。在CRAN中发布的扩展和R程序包往往是最稳定的。因为当一个包在CRAN中发布时，它会经过一个评审过程，从而提高贡献的总体质量。

虽然我们希望能够展示所有的扩展，但是我们将本节限制在你最感兴趣的扩展。你可以在github.com/r-spark部分或通过搜索带有`sparklyr`标记的GitHub仓库找到其它扩展。

* `rsparkling`
    `rsparkling`扩展允许你使用R中的H2O和Spark。此扩展是我们会在Spark中考虑的高级建模方案。虽然Spark的内置建模库Spark MLlib在许多情况下都非常有用，但H2O的建模功能可以计算额外的统计指标，并且可以比Spark MLlib提供更多性能和可扩展性改进。我们既没有对MLlib和H2O进行详细的比较，也没有进行基准测试；这是需要你自己研究的内容，并全面了解何时使用H2O的功能。
* `graphframes`
    `graphframes`扩展增加了对Spark中处理图数据的支持。图是描述一组对象的结构，其中某些对象对在某种意义上是相关的。正如在第1章中所看到的，对网页进行排名是开发MapReduce驱动的Spark初期版本的早期动机；如果你将页面之间的链接视为每对页面之间的关系，则网页碰巧会形成一个图。例如，图上的PageRank等计算操作在网络搜索和社交网络中非常有用。
* `sparktf`
   `sparktf`扩展提供了在Spark中编写TensorFlow记录的支持。TensorFlow是领先的深度学习框架之一，它经常与表示为TensorFlow记录的大量数值数据一起使用，而TensorFlow记录是一种为TensorFlow优化的文件格式。Spark通常用于将非结构化的大规模数据集处理成较小的数值数据集，以便可以很容易地导入GPU。你可以使用这个扩展保存文件格式为TensorFlow记录的数据集。
* `xgboost`
    `xgboost`扩展将著名的XGBoost建模库带到了大规模计算领域。XGBoost是一个可扩展的、可移植的、分布式的梯度增强库。它在解决希格斯玻色子机器学习挑战中获胜后，在机器学习竞赛圈内广为人知，此后在其它kaggle竞赛中也一直很受欢迎。
* `variantspark`
    `variantspark`扩展提供了一个使用Variant Spark这一用于全基因组关联研究（GWAS）的可扩展工具包的接口。它目前提供了构建随机森林模型、估计变量重要性和读取变异判读格式（VCF）文件的功能。虽然Spark中还有其它一些随机森林实现，但大多数都没有针对处理GWAS数据集的优化，这些数据集通常包含数千个样本和数百万个变量。
* `geospark`
   `geospark`扩展使我们能够加载和查询大型地理数据集。通常，包含经纬度点或复杂区域的数据集是Well-Known Text（WKT）格式定义的。WKT是一种文本标记语言，用于表示地图上的矢量几何对象。

在学习如何以及何时使用每个扩展之前，我们应该首先简要介绍如何使用R和Spark扩展。

首先，Spark扩展是接受Spark的R程序包。与其它任何R程序包一样，你首先需要安装扩展。安装好后，一定要注意在使用扩展之前需要重新连接到Spark。所以，一般来说，你应该遵循以下模式：
...
注意，`sparklyr`是在扩展之后加载的，以允许扩展正确注册。如果必须安装并加载新扩展，则首先需要使用`spark_disconnect(sc)`断开连接，重新启动R进程，并针对新扩展重复前面的步骤。

安装和使用R中的Spark扩展并不困难；每个扩展都可以独立掌握，所以大部分时间你需要了解扩展是什么、何时使用以及如何正确使用它。你将学习的第一个扩展是`rsparkling`扩展，它让你能够在Spark和R中使用H2O。

## H2O
    由H2O.ai创建的H2O是一个用于大规模建模的开源软件，它允许你在发现数据模式时拟合数千个潜在的模型。你可以考虑使用H2O来补充或替换Spark的默认建模算法。通常使用Spark的默认建模算法，并在Spark的算法不足或需要高级功能（如其它建模指标或自动模型选择）时过渡到H2O。
  
我们无法在一个段落中公正地介绍H2O强大的建模能力；合理地解释H2O需要一本书。我们建议阅读Darren Cook的《Practical Machine Learning with H2O》（O'Reilly），深入探讨H2O的建模算法和特性。同时，你可以使用这一节作为一个简单的指南，开始在Spark中使用R和H2O。

要使用Spark和H2O，需要知道有四个组件：H2O，Sparkling Water，`rsparkling`和Spark。Sparking Water允许用户将快速、可扩展的H2O机器学习算法与Spark的功能结合起来。你可以将Sparkling Water视为连接Spark与H2O的部分，`rsparkling`视为Sparkling Water的R前端，如图10-1所示。
...
需要注意的是，你需要使用文档中指定的兼容版本的Spark、Sparking Water和H2O。我们提供了Spark 2.3的说明，但使用不同的Spark版本需要安装不同的H2O版本。因此，我们首先通过运行以下命令来检查H2O的版本：
...
然后，我们可以按照如下代码，连接支持的Spark版本（你需要调整`master`参数，以便支持特定的集群）：
...
H2O提供了一个网络界面，可以帮助你监控训练过程并访问H2O的大部分功能。你可以通过`h2o_flow(sc)`访问网络界面（称为H2O流），如图10-2所示。

使用H2O时，你需要使用`as_h2o_frame`将Spark数据框转换为H2O数据框：
...
H2O提供了一些对于Spark建模算法并不提供的额外指标。我们刚刚拟合的模型提供了`Residual Deviance`，而使用Spark MLlib时，这不是一个标准度量。
...
然后，你可以在广义线性模型（GLM）上做预测。类似的方法在H2O的许多其它模型上都适用：
...
对于这个特定的数据集，H2O确定深度学习模型比GLM更适合[^22]。具体来说，H2O的AutoML使用XGBoost、深度学习、GLM和堆栈集成模型等探索数据：
...
还有许多其它示例，你也可以从官方GitHub仓库请求`rsparkling`包的帮助。

下一个扩展`graphframes`允许你处理大型关系数据集。在开始使用之前，请确保使用`spark_disconnect(sc)`断开连接并重新启动R进程，因为使用不同的扩展需要重新连接Spark并重新载入`sparklyr`。

## 图模型

图论史上的第一篇论文是由列昂哈德·欧拉于1736年写的关于柯尼斯堡七桥问题。该问题是要设计一条穿过这座城市的步行道，满足穿过每座桥且只穿过一次。图10-3展示了原始文稿。

![图10-3. 欧拉的柯尼斯堡七桥问题](figures/10_3.png)

今天，一个图被定义为一个有序对$G = (V, E)$，其中$V$是顶点（节点或散点），$E \subseteq \{ \{x, y\}|(x,y) \in V^2 \wedge   x\ne y\}$是边（连接或线）的集合。它们可以是无向图的一个无序对，也可以是有向图的有序对。前者的连接方向不重要，而后者的连接方向是有意义的。

作为一个简单的例子，我们可以使用`ggraph`包中的`highschool`数据集，它记录了高中男生之间的朋友关系。在这个数据集中，顶点是学生，边描述了在某个年份成为朋友关系的学生对：
...
虽然`highschool`数据集可以很容易地在R中处理，但是如果不将工作分布到合适的Spark机器集群中，即使是中等大小的图数据集也很难处理。Spark支持通过`graphframes`扩展来处理图数据，而`graphframes`扩展又使用GraphX Spark组件。GraphX是Apache Spark用于图和图并行计算的API。它在性能上可以与最快的专用图处理系统相媲美，并提供了一个不断增长的图算法库。
...
接下来，我们需要连接Spark，复制`highschool`数据集并将图数据转换为扩展所期望的格式。在这里，我们将这个数据集的范围限定在到1957年的朋友关系：
...
然后我们可以找到哪一条是到某个特定顶点的最短路径（这个数据集中的一个男孩）。由于数据是匿名的，我们可以挑选一个编号为33的男孩，发现数据之间存在多少分离度：
...
最后，我们也可以计算这个图的PageRank值，它在第1章讨论谷歌网页排序算法中提到过。
...
为了有助于理解数据集，图10-4使用`ggraph`画出了这个图，并高亮了下列数据集中最高PageRank得分：
...
![图10-4. `highschool`数据集`ggraph`可视化，其中最高PageRank得分被高亮](figures/10_4.png)

`graphframes`还提供了更多的图算法，例如广度优先搜索、连接分支、用于检测社区的标签传递、强连接分支和三角形计数。有关此扩展的问题，请参阅官方GitHub仓库。我们接下来介绍一个流行的梯度提升框架。确保在尝试下一个扩展之前断开连接并重新启动。
...
XGBoost是一个开源软件库，提供了一个梯度提升框架。它旨在为训练梯度提升决策树（GBDT）和梯度提升回归树（GBRT）提供可扩展、可移植和分布式的梯度提升方法。梯度提升意味着XGBoost使用梯度下降和梯度提升，这是一种按顺序选择预测器的技术。

`sparkxgb`是一个扩展，你可以使用它在Spark中训练XGBoost模型。但是，请注意，当前不支持Windows。要使用此扩展，请首先通过CRAN安装它：

```{r}
install.packages("sparkxgb")
```

然后，你需要导入`sparkxgb`扩展，执行常用的Spark连接代码，按需调整`master`：
...
在本例中，我们使用`rsample`包中的`attrition`数据集。你需要使用`install.packages("rsample")`来安装它。这是一个虚构的数据集，由IBM数据科学家创建，用来揭示导致员工离职的原因：
...
要在Spark中构建XGBoost模型，使用`xgboost_classifier()`。我们使用`Attrition ~ .`公式计算所有其它特性对离职的影响，并指定为二分类问题，因为离职属性只记录员工是离开还是留下。然后，可以使用`ml_predict()`对大规模数据集进行预测：
...
在希格斯玻色子机器学习挑战赛中，胜出的解决方案使用了XGBoost。之后，XGBoost在竞赛圈中广为人知，该方案使用ATLAS实验来识别希格斯玻色子。从此，XGBoost成为一种流行的模式，并应用到大量的kaggle比赛中。然而，决策树被证明能力很有限，特别是在具有非表格数据（如图像、音频和文本）的数据集问题上。而这些问题使用深度学习技术效果会更好。提醒一下，记得断开连接并重新启动。

## 深度学习

感知机是Frank Rosenblatt提出的一种数学模型[^23]，他把感知机发展成为一种假想的神经系统的理论。感知机将刺激映射到数值输入，数值输入加权到阈值函数中，阈值函数仅在受到足够的刺激时激活，数学表示如下：
...
Minsky和Papert发现，一个感知机只能对线性可分的数据集进行分类；然而，他们在其著作《Perceptrons》中也揭示了分层感知机将带来额外的分类能力[^24]。图10-6给出了多层感知机最初的样子。
...
使用Spark，我们可以创建一个多层感知机分类器，该分类器使用`ml_multilayer_perceptron_classifier()`和梯度下降来对大型数据集进行分类和预测。Geoff Hinton将梯度下降引入分层感知机[^25]。
...
注意，列必须是数值型的，因此需要使用第4章中介绍的特征变换技术手动转换它们。尝试添加更多的层来分类更复杂的数据集是很自然的。但是，添加太多的层将导致梯度散失，因此还需要其它技术来维持这些深层网络，也称为*深度学习模型*。
...
你可以使用Spark中的`sparktf`编写TensorFlow记录，并在GPU实例中使用Keras或TensorFlow等库来处理这些记录。

然后，你可以在Spark中预处理大型数据集，并使用`spark_write_tf()`将它们作为TensorFlow记录写入：
...
使用Keras或TensorFlow训练数据集后，可以使用`tfdatasets`包加载它。你还需要使用`install_tensorflow()` 安装TensorFlow runtime，并自行安装Python。要了解更多关于使用Keras训练深度学习模型的信息，建议阅读《Deep Learning with R》[^26]。
...
使用一个或多个GPU在单个本地节点上训练深度学习模型对于大多数应用程序来说通常已经足够了；但是，一些最先进的深度学习模型会使用Apache Spark等分布式计算框架进行训练。分布式计算框架用来实现更高的千万亿次浮点运算，支持系统每天都在训练这些模型。OpenAI分析了人工智能（AI）和集群计算领域的趋势（如图10-7所示）。从图中可以明显看出，近年来有使用分布式计算框架的趋势。

![图10-7. 基于OpenAI分析使用分布式系统的训练](figures/10_7.png)

图中文字：

* Distributed。。。learning，分布式深度学习
* petaflops，千万亿次浮点
* distributed，分布式
* local，本地


通过像Horovod这样的框架，Spark和TensorFlow可以训练大规模的深度学习模型。今天，我们可以通过`reticulate`在R中使用Horovod和Spark。Horovod需要Python和Open MPI，这超出了本书的范围。接下来，我们将介绍基因组学领域的另一个不同的Spark扩展。


## 基因组学

人类基因组由23对染色体中约30亿对碱基DNA的两个拷贝组成。图10-8展示了基因组形成的染色体。DNA链由核苷酸组成，每个核苷酸由四个含氮碱基中的一个组成：胞嘧啶（C）、鸟嘌呤（G）、腺嘌呤（A）或胸腺嘧啶（T）。由于所有人类的DNA几乎是相同的，我们只需要以一个变异判读格式（VCF）文件的形式存储与参考基因组的差异。

![图10-8. 理想化的人类二倍体核型，显示基因组形成染色体](figures/10_8.png)

`variantspark`是一个基于Scala和Spark的分析基因组数据集的框架。它是由澳大利亚的CSIRO生物信息学团队开发的。`variantspark`在3000个样本的数据集上进行了测试，每个样本包含8000万个非监督聚类方法或监督式应用（如分类和回归）的特征。`variantspark`可以读取VCF文件并运行分析，同时使用熟悉的Spark数据框。

首先，从CRAN安装`variantspark`，连接到Spark，然后得到`variantspark`的`vsc`连接：
...
`variantspark`使用随机森林为每个测试的变异分配一个重要分数，反映其与兴趣表型的关联。一个重要度得分较高的变异意味着它与感兴趣的表型有紧密的联系。你可以计算重要性并将其转换为Spark表，如下所示：
...
然后，你可以使用`dplyr`和`ggplot2` 转换输出，并给出分析（见图10-9）：
...
* Importance Analysis，重要性分析
* Genomic。。。variantspark，使用`variantspark`的基因组重要性分析
* variable，变量
* importance，重要性

我们在Spark中使用`variantspark`扩展，简要介绍了基因组分析。接下来，我们将从微观基因数据转移到宏观数据集，这些数据集包含世界各地的地理位置。

## 空间数据

`geospark`使用与`dplyr`和`sf`兼容的语法来支持分布式地理空间计算。该语法提供了一组用于处理地理空间矢量的工具。
...
你还可以通过收集整个数据集的子集或收集之前在Spark中聚合几何图形来绘制这些数据集。一个值得学习的包是`sf`。
...
你还可以考虑从以下扩展仓库中新开一个GitHub issue，以获得扩展作者的帮助：
...
本章简要介绍了R中提供的一些Spark扩展，使用它们与安装软件包一样简单。然后，你学习了如何使用`rsparkling`扩展，该扩展提供了Spark中的请求访问，进而提供了其它建模功能，如增强度量和自动选择模型的能力。然后我们跳到`graphframes`，这个扩展可以帮助你处理图的关系数据集。你还学习了如何计算简单的连接度量或运行复杂的算法，如`PageRank`。

XGBoost和深度学习部分提供了使用梯度下降的备选建模技术：前者基于决策树，后者基于多层感知机。我们可以使用Spark将数据集预处理为记录，然后使用`sparktf`扩展支持TensorFlow和Keras使用。最后两部分介绍了通过`variantspark`和`geospark`扩展来处理基因组和空间数据集。

这些扩展，以及更多的扩展，提供了一个具备高级功能的综合库。结合所介绍的分析和建模技术，这些扩展应该可以涵盖集群运行中所需的大多数任务。然而，当缺少功能时，你可以考虑编写自己的扩展，这是我们在第13章中要讨论的。或者，马上在第11章中可以看到，你可以使用R代码对每个分区进行自定义转换。
...
你可以在每个工作节点中运行R代码来执行任何计算，可以运行模拟、从网络抓取内容、转换数据等。此外，你还可以使用CRAN中可用的任何包和你所在组织中可用的私有包，这将减少编写的代码量，帮助维持生产效率。

如果你已经熟悉R，那么可能会尝试将此方法用于所有Spark操作。但是，这并不意味着建议使用`spark_apply()`。前几章提供了更有效的技术和工具来解决众所周知的问题；相反，`spark_apply()`引入了额外的认知开销、额外的故障排除步骤、性能权衡，并且通常应该避免更多的复杂性。并不是说不应该使用`spark_apply()`，而是让`spark_apply()`对付以前的工具和技术所不能满足的用例。


## 概述

第1章介绍了MapReduce可以作为一种能够处理大规模数据集的技术。它还介绍了Apache Spark如何提供一个操作超集，以便更轻松、更高效地执行MapReduce计算。第9章通过对分布式数据集的每个分区应用自定义转换，深入阐述了Spark是如何工作的。例如，如果我们将一个分布式数值数据集的每个元素重复10次，Spark将通过多个工作节点在每个分区上应用一个映射操作。这个过程的概念图如图11-1所示。

![图11-1. 重复10次的映射操作](figures/11_1.png)
...
我们可以参考第一章中最初的MapReduce示例，其中map操作被定义为将句子拆分为单词，并且对所有唯一单词计数当为reduce操作。
...
* *建模*
使用Spark和R中已有的丰富建模功能是很自然的。在大多数情况下，R模型不能跨大数据使用；但是，我们会专门展示两个用例来说明其中的R模型可以在大数据上使用。例如，当数据适合一台机器时，可以使用网格搜索并行优化它们的参数。在一些情况下，我们可以对数据进行分区，并在数据子集上创建多个模型，我们可以使用R中的分区建模来跨分区计算模型。
* *变换*
你可以使用R的丰富数据转换功能来扩充Spark。我们会展示一个通过外部系统评估数据的用例，并使用R通过web API调用它们来进行交互。
* *计算*
当你需要在R中执行大规模计算，或者如第1章所述执行大型计算时，Spark是分发这类计算的理想选择。我们会模拟R中大规模计算的一个特殊用例。

当详细研究每个用例时，我们会提供一个工作示例，帮助了解如何有效地使用`spark_apply()`。

### 定制解析器

尽管Spark及其各种扩展提供了对许多文件格式（csv、JSON、Parquet、AVRO等）的支持，但是在大规模数据上你可能需要其支持其它格式。你可以使用`spark_apply()`和许多现有的R程序包解析这些额外的格式。在本节中，我们将研究如何解析日志文件。使用类似的方法你还可以解析其它格式的文件。
...
自定义解析器的讨论到此结束；你可以按照类似的方法用R解析大数据的许多其它文件格式。接下来，我们把当前的分区建模看作另一个用例，关注跨多个数据集的并行建模。

### 分区建模

R中有许多可以运行在大数据上的建模包。它们将数据划分成易于管理的几组数据，每组数据都可以放在单个机器上。

例如，假设你有一个1TB的数据集，包含多个城市的销售数据，并且你的任务是在每个城市构建销售预测。在这种情况下，可以考虑将原始数据集划分为10GB的单个城市数据，这些数据可以由单个计算实例管理。对于这种可分区的数据集，可以考虑使用`spark_apply()`，为每个城市训练一个模型。

作为分区建模的简单示例，我们可以使用按品种分区的`iris`数据集进行线性回归：
...
然后，你可以使用`spark_apply()`在每个品种上执行线性回归：
...
![图11-3. `iris`数据集在不同品种上的建模结果](figures/11_3.png)

图中文字：

* Modeling。。。species，在品种上建模
* Petal。。。species，花瓣长度和宽度在品种上建模

我们简单介绍了如何在几个不同的可分区数据集上执行建模。类似的技术可以应用于同一个数据集上的使用不同参数的建模，下面就会介绍。

### 网格搜索

许多R程序包提供了需要定义多个参数完成配置和优化的模型。当这些参数的值未知时，我们可以将一列未知参数发放到一组机器上，以找到最佳的参数组合。如果这列参数包含多个要优化的参数，则通常会通过参数A和参数B之间的所有组合进行测试，从而创建一个参数网格。在这个参数网格上搜索最佳参数的过程通常称为*网格搜索*。
...
Web API是一个程序，可以允许其它程序重用，做一些有用的事情。例如，Twitter之类的服务提供了Web API，允许你用R和其它编程语言编写程序自动读取推文。通过使用R代码向外部服务发送实际请求，可以使用`spark_apply()`调用Web API。

例如，谷歌提供了一个Web API来使用深度学习技术标注图像；你可以通过R使用这个API。但是对于更大的数据集，你需要从Spark访问它的API。你可以使用Spark为Web API准备数据，然后使用`spark_apply()`完成调用并在Spark中处理所有传入的结果。

下一个示例使用`googleAuthR`包向谷歌云完成身份验证，使用`RoogleVision`包在谷歌视觉API上执行标记，使用`spark_apply()`在Spark和谷歌的深度学习服务之间进行交互。要运行以下示例，首先需要断开与Spark的连接，并从谷歌开发者站点下载`cloudml.json`文件：
...
要在Web API上成功运行大型分布式计算，API需要能够扩展以支持来自所有Spark执行器的负载。我们相信主流服务供应商可能支持从集群传入的所有请求。但是，当你调用内部Web API时，请确保API能够处理负载。此外，当使用第三方服务时，需要考虑跨集群中的所有执行器调用API的成本，以避免潜在的，昂贵和意外的费用。
...
在更高的分辨率，例如$1920 \times 1080$中，上一个示例需要几分钟来渲染图11-4中的单个帧；单个机器以每秒30帧的速度渲染将需要数小时。但是，我们可以通过在多台机器上并行计算来减少这一时间。例如，使用10台具有相同数量CPU的计算机可以将渲染时间缩短为十分之一：
...
我们已经介绍了`spark_apply()`的一些常见用例，但是你还要针对具体的需求找出其它用例。下一节将介绍技术性概念，达到创建其它用例和有效使用`spark_apply()`的目的。
...
为了提高性能，可以通过`columns`参数显式指定列。这个参数接受最终数据框中期望的类型命名列表。然后，我们可以通过给`numbers`列指定正确的类型，重写之前的示例，并只运行一次`spark_apply()`：
...
我们已经介绍了行和列如何与`spark_apply()`进行交互，接下来我们将继续使用Context信息。有时在处理分布式数据集时会需要Context信息。

## Context

如果要使用`spark_apply()`处理分区，可能需要包括足以放入每个节点的小型备用数据。网格搜索的例子中就是这样，数据集被传递到所有分区，并且自身保持未分区状态。
...
图11-5展示了这个例子的概念图。注意到数据分区还是可用的，但是`context`参数分配给了所有节点。

![图11-5. 乘以`context`参数的Map操作](figures/11_5.png)
...
你会发现`context`参数非常有用；例如，下一节会介绍如何正确构造函数，并且`context`在高级示例中可以用于构造依赖于其它函数的函数。

## 函数

前面我们将`spark_apply()`作为函数或表达式执行自定义转换的操作。在有关编程的文献中，带有context的函数也被称为*闭包*。

表达式可用于定义语句较短的转换，如`~ 10 * .x`。对于表达式，`.x`包含分区而`.y`包含context信息（如果可用）。但是，我们很难为跨多行的复杂代码定义表达式。对于这些情况，函数更合适。

函数支持复杂的多行转换，并定义为`function(data, context) {}`，你可以在`{}`中提供任何代码。我们在前面的章节中看到过使用谷歌云将图片转换为图片标题的例子。

传递给`spark_apply()`的函数使用`serialize()`进行序列化，`serialize()`被描述为“一个用于序列化连接的简单低级别接口”。`serialize()`的一个当前限制是它不会序列化在其环境之外引用的对象。例如，以下函数由于闭包引用了`external_value`而报错：
...
> 由于在`spark_connect()`连接期间只复制一次包，因此在连接处于活动状态时不支持安装其它包。如果需要安装新程序包，请用`spark_disconnect()`断开连接，修改软件包，然后重新连接。此外，R程序包不在本地模式下复制，因为程序包已经存在于本地系统上。

尽管本节很简短，但是使用分布式R代码的程序包打开了一个全新而有趣的用例场景。本章介绍了其中一些用例，通过寻找当今丰富的R包生态系统，你会发现更多的用例。

本节完成了用程序包分发R代码所需功能的讨论。现在，我们讨论一下集群使用`spark_apply()`时的一些需求。

## 集群需求

前面章节中介绍的功能不需要特殊配置Spark集群。只要你有一个正确配置的Spark集群，就可以使用R。但是，对于下面提供的功能，你的集群管理员、云供应商，或者你自己必须通过安装以下任意一项来配置集群：

* 每个节点中的R，以便在集群中执行R代码
* 当使用Spark 2.3或更高版本时，每个节点中的Apache Arrow（Arrow提供了性能改进，使分布式R代码更接近原生Scala代码）

让我们看看每个需求，确保你可以正确考虑它们提供的利弊。

### 安装R

对于第一个需求，R runtime应该预先安装在集群的每个节点中；这是针对`spark_apply()`的需求。

如果未能在每个节点中安装R，在尝试使用`spark_apply()`时会触发一个`Cannot run program, no such file or directory`的报错。

请与集群管理员联系，保证整个集群中的R runtime可用。如果已经安装了R，可以指定与`spark.r.command`配置一起使用的安装路径，如下所示：
...
由于驱动节点会向工作节点分发包，也可能编译包，因此我们需要同构集群。例如，驱动节点和工作节点必须具有相同的处理器体系结构、系统库等。这通常是大多数集群的情况，但是你的集群可能不是这样。

不同的集群管理器、Spark发行版和云供应商支持不同的解决方案，以便在集群中的每个节点上安装额外的软件（如R）。在每个工作节点上安装R时，请遵循说明。这里有一些示例：
...
虽然Apache Arrow不是必需的，但是还是强烈建议你在使用`spark_apply()`时使用它。它支持Spark 2.3.0以后的版本；但是，它要求系统管理员在每个节点中安装Apache Arrow runtime（请参见http://Arrow.Apache.org/install/）。
...
`arrow`中的大多数功能只是在后台工作，可以提高性能和数据序列化；但是，有一个设置需要注意。`spark.sql.execution.arrow.maxRecordsPerBatch`配置指定每个arrow数据传输的默认大小。它与其它Spark组件共享，默认为10000行：
...
你可能需要根据系统能够处理的数据量调整这个数字，使其对于大型数据集更小，或者对于处理全量记录的操作更大。我们可以将此设置更改为5000行，并验证分区是否也正确更改：
...
除其它适用于`spark_apply()`的更高级故障排除技术外，以下各节按顺序介绍这些技术。你应该首先尝试使用工作日志进行故障排除，然后识别分区错误，最后尝试调试工作节点。
...
此功能在故障排除时非常有用；例如，如果使用`stop()`函数强制执行错误：
...
当你运行几百个执行器时，某些任务可能会无限期挂起。在这种情况下，工作中的大多数任务都会成功完成，但其中有少数任务仍在运行，不是失败，也不是成功。
...
因此，在本例中，你可以配置比这个中位数长四倍的任务，让Spark重试，如下所示：
...
在集群中执行此操作时，可以在日志中查找未完成的任务。一旦找到了摘要信息，你就可以取消这个作业。然后，可以使用摘要检索具体的R数据框，类似如下：
...
如这些说明所示，你需要从不同的R进程中连接“as the worker node”，然后逐步完成代码。这个方法并没有以前的方法直接，因为你还需要查看一些`sparklyr`代码。因此，我们只建议你最后使用这个方法。（你也可以尝试第2章中描述的在线资源。）

现在，让我们在这一章结束时简要回顾一下介绍的功能。

## 小结

本章介绍了`spark_apply()`这一高级技术，你可以使用它来填补Spark或许多扩展中的功能空白。我们给出了`spark_apply()`解析数据、并行建模许多小数据集、执行网格搜索和调用Web API的用例。你还了解到了分区与`spark_apply()`的关系，以及如何创建自定义组、跨所有节点分发context信息，以及排除故障问题、功能局限和集群配置注意事项。我们还强烈建议在使用Spark和R时使用Apache Arrow作为库，并介绍了安装过程、用例和应该注意的事项。

到目前为止，我们只处理了静态数据的大型数据集。这些数据集不会随时间变化，并且在我们分析、建模和可视化它们时保持不变。在第12章中，我们将介绍另一种处理数据集的技术，这些数据集除了很大之外，还像信息流一样增长。
...
回顾前几章，我们已经讨论了很多技术，但还不是全部技术。我们分析了表格数据集，对原始文本执行了无监督学习，分析了图数据和地理数据集，甚至用自定义的R代码转换了数据！那现在还需要学习什么？

在此之前,尽管我们没有明确说明，我们一直假设数据是静态的，并且不会随着时间的推移而改变。但是现在假设你的工作是分析交通模式，向交通部提出建议。一个合理的方法是分析历史数据，然后设计预测模型，在一夜之间完成预测。真的可以一夜之间完成吗？如果真的可以那就很有用，但是交通模式会随着小时甚至分钟的变化而变化。你可以预处理和预测的越来越快，但最终模型还是失效。因为你无法加载大型数据集、转换数据、对数据进行评分、撤回数据，并在每一秒重复此过程。

相反，我们需要引入一种不同的数据集，一种不是静态的而是动态的，类似于表但不断增长的数据集。我们将这些数据集称为*数据流*。

## 概述

虽然我们知道如何处理大规模的静态数据集，但是我们如何推理大规模的实时数据集呢？具有无限条目数的数据集称为数据流。

对于静态数据集，如果我们使用预先训练的主题模型进行实时评分，记录就是文本行；对于实时数据集，我们会对无限行的文本执行相同的评分操作。在实践中，你其实永远不会处理无限多的记录。你最终会终止这条数据，何况这个世界也会终结。这两个结果哪个先到还不知道呢。不管怎样，将数据集看成有限数据集让我们更容易对它们进行推理。

数据流是处理实时数据最常用的技术，例如分析Twitter feed或股票价格。这两个示例都有定义良好的列，如“tweet”或“price”，但总是有新的数据行要分析。

*Spark Streaming*提供了可扩展和容错的数据流处理方法。这意味着你可以使用多台计算机来处理多个流数据源，与其它流或静态源连接，并保证至少从故障中恢复一次（每条消息都会被传递，但可能会传递多次）。

在Spark中，通过定义*源*、*转换*和*sink*来创建流；可以将这些步骤看作是读取、转换和写入数据流，如图12-1所示。
...
* 读取
    数据流使用`stream_read_*()`函数读取数据；读取操作定义流的源。你可以定义一个或多个要读取的源。
* 转换
    数据流可以使用`dplyr`、`SQL`、特征变换、评分管道或分布式R代码执行一个或多个转换。转换不仅可以应用于一个或多个流，还可以用于流和静态数据源的组合。例如，用`spark_read_()`函数给Spark加载数据，其转换意味着你可以轻松地组合静态数据和实时数据源。
* 写入
    读取操作是通过流的一系列`stream_read_*()`函数来执行，而写入操作定义了流的sink。你可以指定一个或多个sink来写入数据。
...
数据流开始以`stream_read_*()`运行。一旦执行，数据流将监视`source`路径，并在数据到达时处理至`destination/`路径。

我们可以使用`stream_generate_test()`每秒生成一个文件，其中包含服从给定分布的文本行；你可以在附录A中阅读更多有关此内容的信息。

在实践中，你会连接到现有的源，而不必人工生成数据。然后，我们可以使用`view_stream()`跟踪数据源和目标路径中每秒处理的行数（rps）及其随时间变化的最新值：
...
注意，目标地址流中的rps速率高于数据源流中的rps速率。由于Spark测量来自数据源流的传入速率，而且还测量目标流中的实际行处理次数，因此这是符合预期的，也是期望的。例如，如果每秒向`source/`路径写入10行，则传入速率为10 rps。但是，如果Spark只需要0.01秒就可以写入这10行，那么输出速率是100 rps。
...
上述练习介绍了如何轻松启动Spark数据流，并基于模拟的数据流读取和写入数据。下面让我们通过转换做一些比复制数据更有趣的事情。
...
转换的数据源可以是数据流，也可以是数据框，但输出始终是数据流。如果需要，可以始终从目标流中获取快照，然后保存输出为数据框。如果未指定目标流，`sparklyr`会负责这些事情。

下面的每个小节都介绍了`sparklyr`提供的在数据流上执行转换的选项。

### 分析

你可以使用`dplyr`操作和`DBI`的SQL分析数据流。作为一个快速示例，我们会在数据流上筛选行和添加列。我们不会显式调用`stream_generate_test()`。但是如果你觉得需要，可以自己调用，进而核实数据正在持续处理：
...
你还可以在数据流的整个历史记录中执行聚合。历史记录可以经过筛选，也可以不经过筛选：
...
尽管如此，还有其它的建模概念可以与数据流一起使用，如特征变换器和评分。让我们用数据流尝试一个特征变换器，并将评分留给下一节，因为我们需要训练一个模型。

下一个示例使用`ft_bucketizer()`特征变换器修改数据流，然后是常规的`dplyr`函数，你可以像使用静态数据集一样使用这些函数：
...
要给数据流评分，必须首先创建我们的模型。因此，让我们构建、调试并保存一个简单的管道：
...
> **如果你愿意，可以使用第5章中介绍的其它概念，例如对数据流进行评分之前，通过`ml_save()`和`ml_load()`保存和重新加载管道。**
...
正如可以预见的，它通过运行自定义的`round()`函数将来自`cars-stream`的数据处理成`cars-round`。让我们看看输出：
...
Apache Kafka是领英开发的开源数据流处理软件平台，并捐赠给了Apache软件基金会。它是用Scala和Java编写的。做一个类比的话，Kafka是实时存储，Hadoop是静态存储。

Kafka将数据流存储为记录，记录由键、值和时间戳组成。它可以按主题对包含不同信息的多个流进行分类，并处理这些数据流。Kafka通常用于连接多个实时应用程序。*生产者*是将数据流到Kafka的应用程序，而*消费者*是从Kafka读取数据的应用程序；在Kafka术语体系中，消费者应用程序*订阅*主题。因此，我们可以用Kafka完成的最基本的工作流是，包含一个生产者和一个消费者的流程，如图12-3所示。
...
输入的字母推到Kafka，被Spark读取，在Spark中聚合，然后返回给Kafka。最后，它们再次被Spark使用，提供有关`totals`主题的简单信息。这是一个很好的流程，也是实时数据处理项目中常见的实际配置。

接下来，我们将使用Shiny框架来实时可视化数据流！

## Shiny

Shiny的reactive框架非常适合支持流式信息，你可以使用`reactiveSpark()`显示来自Spark的实时数据。Shiny的知识远比我们在这里所能了解到的多得多。如果你已经熟悉Shiny，那么这个示例应该很容易理解。
...
当Shiny应用程序运行时，会从同一目录启动一个新的R进程，并使用`stream_generate_test()`创建一个测试流。这会生成Spark可以处理的和Shiny可以可视化的连续数据流（如图12-5所示）：
...
在这一部分中，你知道了创建一个可用于多种不同目的（如监视和仪表板）的Shiny应用程序是非常容易的。
...
从静态数据集到实时数据集，你已经真正掌握了许多大规模计算技术。具体来说，在本章中，你学习了将静态数据视为一个无限表的情况下，如何将其推广为实时数据。然后我们可以创建一个简单的数据流，无需任何数据转换，就可以将数据从A复制到B。

当你了解到不同的转换可以用于数据流时，这个简单的开始也很有用。例如，可以使用`dplyr`和`DBI`程序包完成数据分析，建模时引入的特征变换器、能够实时评分的成熟管道，以及使用自定义R代码转换数据集。当然，这些内容需要消化。

然后，我们将Apache Kafka作为一个可靠的、可扩展的实时数据解决方案，展示了如何通过消费者、生产者和主题来构建实时系统。如果将它们适当地组合在一起，可以构建出强大的体系架构来处理实时数据。

然后我们介绍了锦上添花的技术，如何在Shiny中使用Spark Streaming。由于数据流可以转换为reactive类型（响应式编程领域的通用语言），因此其易用性绝对让人惊喜。现在是时候进入我们的（非常简短的）最后一章了，第13章。我们会试图说服你使用新知识，让整个Spark和R社区的获益。
...
在第12章中，我们提供了使用R处理Spark中大规模实时数据的工具。在最后一章中，我们将更少地关注学习，更多地关注回报Spark和R社区或你职业生涯中的同事。这真的需要整个社区来维持，所以我们需要你！

有很多方法可以做出贡献，从帮助社区成员和新开一个GitHub issue，到为自己、同事或R和Spark社区提供新功能。但是，这里我们只关注编写和共享扩展Spark的代码，以帮助其它人使用新的功能。你可以成为这些Spark扩展的作者。具体来说，你可以从零开始，了解什么是扩展、可以构建的不同扩展类型、可用的构建工具，何时以及如何构建扩展。

你还会学习到如何利用Spark中提供的数百个扩展和Java中提供的数百万个易于在R中使用的组件。我们还会介绍如何构建使用Spark的Scala代码。你会看到，R是与其它语言如C++、SQL、Python等交互的伟大语言。因此，使用R中的Scala也就不足为奇了。它会遵循类似的实践，使R成为提供易用接口的理想选择。这些接口使数据处理变得高效，并且深受用户喜爱。

## 概述

当你考虑回馈更大的编程社区时，对于你所写的任何一段代码，你能问的最重要的问题是：这段代码对其它人有用吗？
...
这个基础代码可能对其它人没有用处。但是，你可以将同样的示例调整为能够产生更多意义的内容，可能如下所示：

```{r}
spark_read_csv(sc, "/path/that/is/hard/to/remember/data.csv")
```

这条代码与第一条很类似。但是如果你和其它人一起使用这个数据集，这条代码是有用的。有点意外！因为这意味着并非所有有用的代码都很高级或复杂。但是，为了让它对其它人有用，它确实需要以易于使用的格式打包、呈现和共享。

第一次使用时将其保存到`teamdata.R`文件中，并编写一个函数封装它：
...
这是一个改进，但需要用户一次又一次地手动共享此文件。幸运的是，这个问题很容易在R中通过R程序包解决。R程序包包含R代码，支持使用`install.packages()`函数安装。其中一个例子是`sparklyr`。还有许多其它R包可用；你也可以创建自己的R包。对于创建R包的新手，我们鼓励阅读Hadley Wickham的著作《R Packages》（O'Reilly）。创建R包允许你在组织中共享包文件，进而轻松地与其它人共享函数。

一旦你创建了一个包，有很多方法可以与同事或外界分享。例如，对于自己使用的包，可以考虑使用Drat或RStudio包管理器之类的产品。对于公开使用的R包，可以提供给CRAN（R综合档案网络）的R社区。

这些R包仓库允许用户通过`install.packages("teamdata")`安装程序包，而不必担心从何处下载。它还允许其它包重用你的包。除了使用R包（如`sparklyr`、`dplyr`、`broom`和其它包）创建扩展Spark的新R包外，你还可以使用Spark API或Spark扩展中提供的所有功能，也可以编写自定义Scala代码。

例如，假设有一个类似于CSV但不完全相同的新文件格式。我们可能需要编写一个名为`spark_read_file()`的函数，该函数将接收这个新文件类型的路径，并在Spark中读取它。一种方法是使用`dplyr`处理文本的每一行，或借助`spark_apply()`的其它R包。另一种方法是使用Spark API访问Spark提供的方法。第三种方法是检查Spark社区中是否已经有人提供了支持这种新文件格式的Spark扩展。最后，你可以编写自己定制的Scala代码，使用任何Java库，包括Spark及其扩展。如图13-1所示。
...
很简单：我们使用`spark_read_text()`读取整个文本文件，然后使用`dplyr`的`count()`计数行。现在，假设`spark_read_text()`、`dplyr`或任何其它Spark功能都不可用。你会要求Spark如何计算`cars.csv`中的行数？如果在Scala中执行此操作，可以在Spark文档中找到答案。通过使用Spark API，可以按如下方式计算文件中的行数：
...
`invoke()`函数最初设计用于调用Spark代码，但它也可以调用Java中的任何代码。例如，我们可以使用以下代码创建一个Java的`BigInteger`：
...
例如，有一个Apache Solr，它是一个用于在大型数据集上执行全文搜索的系统，Apache Spark目前并不原生支持这种功能。另外，在本文撰写之时，还没有R扩展来支持Solr。所以，让我们试着用Spark扩展来解决这个问题。

首先，如果搜索“spark packages.org”来找到Solr扩展，那么你应该能够找到`spark-solr`。“How to”扩展指出应该加载`com.lucidworks.spark:spark-solr:2.0.1`。我们可以使用`sparklyr.shell.packages`配置选项在R中完成配置：
...
虽然指定`sparklyr.shell.packages`参数通常就足够了，但是对于这个特定的扩展，依赖项无法从Spark包仓库下载。你需要在Maven repo中手动查找失败的依赖项，并在`sparklyr.shell.repositories`参数下添加更多的仓库。

> 当使用扩展时，Spark会连接到Maven包仓库来以便获取扩展。这可能需要很长时间，取决于扩展和你的下载速度。在这种情况下，考虑使用`sparklyr.connect.timeout`配置参数来允许Spark下载所需的文件。

从`spark-solr`文档中，你可以发现这个扩展可以通过以下Scala代码使用:
...
但是，这段代码会失败，因为它需要一个有效的Solr实例，并且配置Solr超出了本书的范围。但是这个例子帮你理解如何创建Spark扩展。值得一提的是，你可以使用`spark_read_source()`从通用源中读取，以避免编写自定义的`invoke()`代码。

正如“概述”中指出的，考虑与使用R包的其它人共享代码。虽然可以要求你的包的用户指定`sparklyr.shell.packages`，但是在R包中注册依赖项可以避免这种情况。依赖项在`spark_dependencies()`函数下声明；因此，对于本节中的示例：
...
当加载库时，`onLoad`函数由R自动调用。它会调用`register_extension()`，该函数再回调`spark_dependencies()`，以允许扩展提供其它依赖项。本例支持Spark 2.4，但也应该支持将Spark和Scala版本映射到正确的Spark扩展版本。

你可以使用大约450个Spark扩展；此外，你还可以使用Maven仓库中的任何Java库，其中Maven Central有超过300万个组件。虽然并不是所有的Maven Central库都与Spark相关，但是Spark扩展和Maven仓储库的结合无疑为你提供了许多值得考虑的可能性！

但是，对于那些没有Spark扩展的情况，下一节将介绍如何使用自己R包定制Scala代码。

## 使用Scala代码

Scala代码允许你使用Spark API、Spark扩展或Java库中的任何方法。此外，在Spark中运行编写的Scala代码可以在使用`spark_apply()`的R代码时提供性能改进。通常，R包结构会包含R代码和Scala代码；但是，Scala代码需要编译为JAR包（Java归档文件）并包含在程序包中。图13-2展示了这种结构。

![图13-2. 使用Scala代码时的R程序包结构](figures/13_2.png)
...
通常，R代码应该放在顶层R文件夹下，Scala代码放在java文件夹下，编译的JAR包放在*inst/java*文件夹下。尽管支持手动编译Scala代码，但是你可以使用帮助函数下载所需的编译器并编译Scala代码。

要编译Scala代码，需要安装Java Development Kit 8（简称JDK8）。从Oracle的Java下载页面下载JDK；这需要重新启动R进程。

你还需要Scala编译器支持Scala 2.11和2.12。使用`download_scalac()`，Scala编译器会自动下载和安装：

```{r}
download_scalac()
```

接下来，需要使用`compile_package_jars()`编译Scala源代码。默认情况下，它使用`spark_compilation_spec()`，并支持以下Spark版本：

```{r}
## [1] "1.5.2" "1.6.0" "2.0.0" "2.3.0" "2.4.0"
```

你还可以使用`spark_compilation_spec()`创建自定义条目来定义这个配置。

虽然可以从头开始为Scala代码创建项目结构，但也可以简单地调用`spark_extension(path)`在给定路径中创建扩展。虽然这个扩展很可能是空的，但是包含调用Scala代码的项目结构。

由于`spark_extension()`在RStudio中注册为自定义项目扩展，你还可以从“File”菜单中使用Scala代码创建扩展Spark的R包；单击“New Project”，然后选择“R Package using Spark”，如图13-3所示。

![图13-3. 从Rstudio创建Scala扩展包](figures/13_3.png)
...
由于JAR包默认编译到*inst/package*路径中，因此在构建R包时，所有JAR包也将包含在程序包中。这意味着你可以共享或发布你的R包，为R用户提供相关功能。对于拥有Scala专业知识的Spark高级用户来说，有必要考虑用Scala为R用户和R社区编写库，然后将这些库打包成易于调取、使用和共享的R包。

如果你对R环境中的Spark扩展开发感兴趣，又在开发过程中遇到困难，可以考虑加入`sparklyr`的Gitter频道。我们中的许多人都会在这里帮助社区成长。我们希望很快听到你的声音！

## 小结

最后一章介绍了一组新的工具，用于将Spark功能扩展到R和R包当前支持的范围之外。这个巨大的仓库包括450多个Spark扩展和数以百万计的Java组件。它们可以帮助我们在R中使用Spark。除了这些资源之外，你还学习了如何使用Scala代码构建Java组件，该代码可以很容易地从R嵌入和编译。

这让我们回到了本书的初衷：我们知道你已经学习了如何在R中使用Saprk执行大规模计算，并相信你已经掌握了通过Spark扩展帮助其它社区成员所需的知识。但是，我们迫不及待地想看到你们的新创造，这肯定会有助于Spark和R社区的发展。

总结一下。我们希望前几章完成了Spark和R的简单介绍。然后，为了使用Spark，通过熟悉的R程序包，我们介绍了分析和建模基础。接着，你学习了如何在合适的Spark集群中执行大规模计算。本书的最后三分之一关注在高级主题上：使用扩展、分发R代码、处理实时数据。最后，借助R和Scala代码通过Spark扩展回馈社区。

我们已经尽所能提供了最好的内容。但是，如果你知道任何改进本书的方法，请在the-r-in-spark仓库下新开一个GitHub issue。我们会在以后的修订中关注你的建议。我们希望你喜欢读这本书，而且你学到的和我们在写这本书时学到的一样多。我们也希望花时间阅读本书是值得的。有你作为我们的读者是我们的荣幸。

# 附录A：补充参考代码

在整本书中，我们都提到了这个附录。这里我们涵盖了重要的内容（并列出了可以找到这些材料的章节）。

## 前言

### 格式化

下面的`ggplot2`主题用来格式化本书的所有绘图：
...
![图A-1. 面向统计计算的R项目](figures/A_1.png)

#### 安装Java

下载Java安装文件（见图A-2），在Windows，Mac或Linux平台上启动。

![图A-2. Java下载页](figures/A_2.png)

Spark 2.1需要Java 8。但是，之前的Spark版本支持Java 7。不管怎么样，我们都建议安装Java Runtime Engine 8（JRE 8）。

> 如果你是一个已经在使用Java Development Kit（JDK）的高级读者，请注意JDK 9+目前不受支持。因此，你需要通过卸载JDK 9+或适当设置`JAVA_HOME`来降级到JDK 8。

#### 安装RStudio

虽然安装RStudio在使用Spark和R并不是必须的，但它会提高工作效率。因此，我们建议还是安装Rstudio。下载RStudio安装程序（参见图A-3），然后在Windows，Mac或Linux平台上启动。

![图A-3. RStudio下载页](figures/A_3.png)

启动RStudio后，你可以使用控制台面板来执行本章中的代码。

#### 使用RStudio

如果你不熟悉RStudio（参见图A-4），应该注意以下面板：
* 控制台
    可以使用一个独立的R控制台来执行本书中介绍的所有代码。
* 程序包
    此面板允许你轻松安装`sparklyr`，检查其版本，导航到帮助内容，等等。
* 连接
    此面板允许你连接到Spark，管理活动连接，并查看可用的数据集。


![图A-4. RStudio界面](figures/A_4.png)
...
名称 | 描述
-------|----------
`size(Map<K.V>)` |返回map类型元素个数。
`size(Array)` |返回数组类型元素个数。
`map_keys(Map<K.V>)` |返回包含输入的map键的未排序数组。
`map_values(Map<K.V>)` |返回包含输入的map值的未排序数组。
`array_contains(Array, value)` |如果数组包含某个值，则返回TRUE。
`sort_array(Array)` |按照数组元素的自然顺序升序排列，并返回数组。
`binary(string or binary)` |把参数映射为二值类型。
`cast(expr as a given type)` |把表达式`expr`的结果转换为给定类型。
`from_unixtime(bigint unixtime[, string format])`|把Unix时间戳（1970-01-01 00:00:00 UTC）的秒数转换为字符串。
`unix_timestamp()` |获取当前Unix的时间戳的秒数。
`unix_timestamp(string date)` |把格式为yyyy-MM-dd HH:mm:ss的时间字符串转换为Unix时间戳（以秒计）。
`to_date(string timestamp)` |返回时间戳字符串的日期部分。
`year(string date)` |返回日期的年份。
`quarter(date/timestamp/string)` |返回日期的所在。
`month(string date)` |返回日期的月份。
`day(string date)`|返回日期的日数。
`hour(string date)` |返回时间戳的小时数。
`minute(string date)` |返回时间戳的分钟数。
`second(string date)` |返回时间戳的秒数。
`weekofyear(string date)` |返回时间戳的周数。
`extract(field FROM source)` |从数据源获取特定字段，例如日期或小时。数据源必须是日期，时间戳，时间段，或者可以转成日期和时间戳的字符串。
`datediff(string enddate, string startdate)`|返回`startdate`与`enddate`之间的天数。
`date_add(date/timestamp/string startdate, tinyint/smallint/int days)`|给`startdate`增加天数。
`date_sub(date/timestamp/string startdate, tinyint/smallint/int days)`|给`startdate`减掉天数。
`from_utc_timestamp(\{any primitive type} ts, string timezone)`|把UTC的时间戳转换到给定时区。
`to_utc_timestamp(\{any primitive type} ts, string timezone)`|把给定时区的时间戳转换到UTC时区。
`current_date` |返回当前日期。
`current_timestamp` |返回当前时间戳。
`add_months(string start_date, int num_months, output_date_format)`|返回`start_date`后`num_months`月数的日期。
`last_day(string date)` |返回日期所在月份的最后一天。
`next_day(string start_date, string day_of_week)`|返回晚于`start_date`的第一个日期，并命名为`day_of_week`。
`trunc(string date, string format)`|返回按照给定格式截断的日期结果。
`months_between(date1, date2)` |返回`date1`和`date2`之间的月数。
`date_format(date/timestamp/string ts, string fmt)`|把日期/时间戳/字符串转换为给定日期格式`fmt`的`string`取值。
`if(boolean testCondition, T valueTrue, T valueFalseOrNull)`|当`testCondition`为真时返回`valueTrue`；否则返回`valueFalseOrNull`。
`isnull( a )` |当a是`NULL`时返回真，否则返回假。
`isnotnull( a )` |当a不是`NULL`时返回真，否则返回假。
`nvl(T value, T default_value)` |当值是`NULL`时返回默认值，否则返回`value`。
`COALESCE(T v1, T v2, …)` |返回第一个不是`NULL`的`v`，如果v都是`NULL`则返回`NULL`。
...
`ascii(string str)` | 返回`str`中第一个字符的数值编码。
`base64(binary bin)`| 把二进制变量转换为base64字符串。
`character_length(string str)` |返回`str`中UTF-8字符的数量。
`concat(stringǀbinary A, stringǀbinary B…)`|返回拼接通过有序传参的字符串或字节的结果，也为字符串或字节格式。例如，`concat(foo, bar)`的结果是`foobar`。
`context_ngrams(array<array>,array, int K, int pf)`|返回句子经切分后的前k个N-gram结果。
`concat_ws(string SEP, string A, string B…)`|类似`concat()`，但是使用定制分隔符`SEP`。
`decode(binary bin, string charset)`|把第一个参数解码为使用给定字符集合（US-ASCII，ISO-8859-1，UTF-8，UTF-16BE，UTF-16LE， 和UTF-16其中的一个）的字符串。如果任何一个参数是`NULL`，则结果也是`NULL`。
`elt(N int,str1 string,str2 string,str3 string,…)`|返回索引数字处的字符串。`elt(2,hello,world)`返回`world`。
`encode(string src, string charset)`|把第一个参数编码为使用给定字符集合（US-ASCII，ISO-8859-1，UTF-8，UTF-16BE，UTF-16LE， 和UTF-16其中的一个）的二进制结果。
`field(val T,val1 T,val2 T,val3 T,…)`|返回`val`（取值来自`val1`,`val2`,`val3`的列表）的索引，若没有找到则返回0。
`find_in_set(string str, string strList)`|返回`str`在`strList`中的第一次出现，其中`strList`是逗号分割的字符串。
`format_number(number x, int d)`|讲数字`x`格式化为诸如'#,###,###.##'的格式，四舍五入`d`个小数点位，并返回`as a string`的结果。如果`d` 是0，则结果没有小数点，也没有小数部分。
`get_json_object(string json_string, string path)`|根据给定的JSON路径，抽取JSON字符串中中的JSON对象，并返回JSON对象的字符串。
`in_file(string str, string filename)`|如果`str`出现在`filename`中，则返回`true`。
`instr(string str, string substr)`|返回`str`中`substr`第一次出现的位置。
`length(string A)` |返回字符串长度。
`locate(string substr, string str[, int pos])`|返回`str`中，`pos`位置之后`substr`第一次出现的位置。
`lower(string A) lcase(string A)`|返回A的所有字符的小写结果。
`lpad(string str, int len, string pad)`|返回经过长度为`len`的`pad`左补全的`str`结果。如果`str`长度大于`len`，则返回截断至`len`长度的字符串。
`ltrim(string A)` |返回删除A左侧空格后的字符串。
`ngrams(array<array>, int N, int K, int pf)`|返回句子切分后前k个N-gram结，例如`sentences()`返回的结果。
`octet_length(string str)` |返回`str`在UTF-8编码下所需的八进制编码的长度。
`parse_url(string urlString, string partToExtract [, string keyToExtract])`|返回URL的特定部分。`partToExtract`的合法取值包括`HOST`， `PATH`，`QUERY`，`REF`，`PROTOCOL`，`AUTHORITY`，`FILE`和`USERINFO`。
`printf(String format, Obj… args)`|返回按照`printf`风格格式化的输入结果。
`regexp_extract(string subject, string pattern, int index)`|返回使用模式抽取的字符串。
`regexp_replace(string INITIAL_STRING, string PATTERN, string REPLACEMENT)`|返回替换`INITIAL_STRING`中所有子串的结果，Java正则表达语法通过`PATTERN`和实例`REPLACEMENT`来定义。
`repeat(string str, int n)` |将`str`重复`n`次。
`replace(string A, string OLD, string NEW)`|返回字符串A经过所有由`OLD`到`NEW`的非重叠替换的结果。
`reverse(string A)` |返回逆向字符串。
`rpad(string str, int len, string pad)`|返回经过长度为`len`的`pad`右补全的`str`结果。
`rtrim(string A)` |返回删除A右侧空格后的字符串。
`sentences(string str, string lang, string locale)`|将自然语言文本字符切分为单词和句子，每一个句子通过合适的句子边界切分，并返回单词数组。
`space(int n)` |返回包含n个空格的字符。
`split(string str, string pat)`| 将`str`按照`pat`切割（`pat`是一个正则表达式）。
`str_to_map(text[, delimiter1, delimiter2])`|使用两个分隔符将文本分成键值对。`delimiter1`将文本分成键值对，`delimiter2`分开每组 键值对。默认的分隔符包括，`delimiter1`的,和`delimiter2`的:。
`substr(string binary A, int start)`|返回子串或字节数组A的片段，其从`start`位置开始直到字符串A结束。
`substring_index(string A, string delim, int count)`|返回字符串A在分隔符`delim`的`count`次出现前的子串。
`translate(stringǀcharǀvarchar input, stringǀcharǀvarchar from, stringǀcharǀvarchar to)`|将`input`字符串中出现在`from`中的字符替换为 `to`字符串中的相应字符。
`trim(string A)` |返回删除A两侧空格后的字符串。
`unbase64(string str)`| 将`base64`字符串转换为`BINARY`。
`upper(string A) ucase(string A)`|返回A的所有字符的大写结果。例如，`upper(fOoBaR)`的结果是`FOOBAR’\`。
`initcap(string A)` |返回首字母为大写，其它字母为小写的字符串。单词使用空格分割。
`levenshtein(string A, string B)`|返回两个字符串的Levenshtein距离。
`soundex(string A)` |返回字符串的`soundex`编码。
`mask(string str[, string upper[, string lower[, string number]]])`|返回`str`的掩码结果。
`mask_first_n(string str[, int n])`|返回前n个字符被掩盖的`str`的掩码结果。
`mask_first_n("1234-5678-8765-4321", 4) `|生成结果`nnnn-5678-8765-4321`。
`mask_last_n(string str[, int n])`|返回后n个字符被掩盖的`str`的掩码结果。
`mask_show_first_n(string str[, int n])`|返回前n个字符未掩盖的`str`的掩码结果。
`mask_show_last_n(string str[, int n])`|返回后n个字符未掩盖的`str`的掩码结果。
`mask_hash(stringǀcharǀvarchar str)`|返回`str`的哈希值。
`java_method(class, method[, arg1[, arg2..]])`|`reflect`等效函数。
`reflect(class, method[, arg1[, arg2..]])`|通过匹配参数签名和反射，调用Java方法。
`hash(a1[, a2…])` |返回参数的哈希值。
`current_user()` |返回经过配置的身份验证程序管理器中的当前用户名。
`logged_in_user()` |返回当前进程状态的用户名。
`current_database()` |返回当前数据库名。
`md5(string/binary)`| 计算字符串或二进制表示的128位MD5校验码。
`sha1(string/binary) sha(string/binary)`|计算字符串或二进制表示的SHA-1摘要，并返回十六进制字符结果串。
`crc32(string/binary)`| 计算字符串或二进制表示的循环冗余校验值并返回bigint值。
`sha2(string/binary, int)`| 计算哈希函数（SHA-224，SHA-256，SHA-384和SHA-512）的SHA-2结果。
`aes_encrypt(input string/binary, key string/binary)`|使用AES对输入加密。
`aes_decrypt(input binary, key string/binary)`|使用AES对输入解密。
`version()` |返回Hive版本号。
`count(expr)` |返回获取的总行数。
`sum(col), sum(DISTINCT col)` |返回列元素的加和或列元素中唯一值的加和。 
`avg(col), avg(DISTINCT col)` |返回列元素的平均值或列元素中唯一值的平均值。
`min(col)` |返回列元素的最小值。
`max(col)` |返回列元素的最大值。
`variance(col), var_pop(col)` |返回数值列的方差。
`var_samp(col)` |返回数值列的无偏采样方差。
`stddev_pop(col)` |返回数值列的标准差。
`stddev_samp(col)` |返回数值列的无偏采样标准差。
`covar_pop(col1, col2)` |返回一对数值列的总体协方差。
`covar_samp(col1, col2)` |返回一对数值列的样本协方差。
`corr(col1, col2)` |返回一对数值列的皮尔逊相关性系数。
`percentile(BIGINT col, p)` |返回数值列的第p个精确分位数（不使用浮点类型）。p必须在0和1之间。
`percentile(BIGINT col, array(p1 [, p2]…))`|返回数值列的第p1，p2等精确分位数。pi必须在0和1之间。
`percentile_approx(DOUBLE col, p [, B])`|返回数值列的第p个近似分位数（使用浮点类型）。参数`B`控制内存成本的近似精确度。值越大，产生的近似越好。默认是10,000。当`col`中唯一值个数小于`B`时，会给出精确分位数。
`percentile_approx(DOUBLE col, array(p1 [, p2]…) [, B])`|同前，但是接受和返回一个分位数数组，而不是单个值。
`regr_avgx(independent, dependent)`|等价于`avg(dependent)`.
`regr_avgy(independent, dependent)`|等价于`avg(independent)`.
`regr_count(independent, dependent)`|返回用于拟合线性回归曲线的非null数值对的数量。
`regr_intercept(independent, dependent)`|返回线性回归曲线在y轴的截距。也就是等式`dependent = a * independent + b`中`b`的值。
`regr_r2(independent, dependent)`|返回回归结果的确定性系数。
`regr_slope(independent, dependent)`|返回线性回归的斜率。也就是等式`dependent = a * independent + b`中`a`的值。
`regr_sxx(independent, dependent)`|等价于`regr_count(independent, dependent) * var_pop(dependent)`。
`regr_sxy(independent, dependent)`|等价于`regr_count(independent, dependent) * covar_pop(independent, dependent)` 。
`regr_syy(independent, dependent)`|等价于`regr_count(independent, dependent) * var_pop(independent)`。
`histogram_numeric(col, b)`| 使用`b`个非均匀分割的分箱，计算数值列的直方图。输出是大小`b`的双值（x，y）坐标数组。其中的元素代表分箱的中心和高度。
`collect_set(col)` |返回去除重复元素的对象集合。
`collect_list(col)` |返回带有重复值的对象列表。
`ntile(INTEGER x)`| 把一个有序划分分成x个名为`bucket`的分桶,并给划分中的每一行指认一个分桶号。这样易于计算三位数、四位数、十位数、百分位数和其他常用汇总统计量。
`explode(ARRAY a)`| 将一个数组转换为多个行。返回单列（`col`）的集合，其中每一行对应数组中的一个元素。
`explode(MAP<Tkey,Tvalue> m)`| 将一个map转换为多个行。返回两列（`(key,value)`）的集合，其中每个键值对对应map中的一个输入。
`posexplode(ARRAY a)`|将一个数组转换为多个行，并带有额外的`int`类型的位置列（原始数组中元素的位置，从0开始）。返回两列（`(pos,val)`）的集合，其中每一行对应数组中的一个元素。
`inline(ARRAY<STRUCT<f1:T1,…,fn:Tn>> a)`|将一个结构体数组转换为多个行。返回N列（`(pos,val)`）的集合（N=结构体中顶层元素数量），其中每一行对应数组中的一个结构体。
`stack(int r,T1 V1,…,Tn/r Vn)`| 把`n`的值`V1`，`Vn`分成r行。每一行有`n/r`列。`r`必须是常数。
`json_tuple(string jsonStr,string k1,…,string kn)`|接受JSON字符串和n个键的集合，返回n个值的元组。
`parse_url_tuple(string urlStr,string p1,…,string pn)`|接受URL字符串和n个URL组件，返回n个值的元组。
...
算法 |函数
------|-------
决策树| `ml_decision_tree_classifier()`
梯度提升分类树 |`ml_gbt_classifier()`
线性支持向量机 |`ml_linear_svc()`
逻辑回归|`ml_logistic_regression()`
多层感知机 |`ml_multilayer_perceptron_classifier()`
朴素贝叶斯 |`ml_naive_bayes()`
一对多分类 |`ml_one_vs_rest()`
随机森林 |`ml_random_forest_classifier()`

#### 回归

算法 |函数
------|-------
加速时效时间生成回归 |`ml_aft_survival_regression()`
决策树 |`ml_decision_tree_regressor()`
广义线性回归|`ml_generalized_linear_regression()`
梯度提升回归树|`ml_gbt_regressor()`
...
算法 |函数
------|-------
二分k-means聚类 |`ml_bisecting_kmeans()`
...
文档集合词表|`ft_count_vectorizer()`
离散余弦转换 |`ft_discrete_cosine_transform()`
使用`dplyr`转换 |`ft_dplyr_transformer()`
Hadamard积|`ft_elementwise_product()`
特征哈希 |`ft_feature_hasher()`
使用哈希的词频 |`export(`ft_hashing_tf)
逆文档频率 |`ft_idf()`
缺失值推断 |`export(`ft_imputer)
索引字符化 |`ft_index_to_string()`
特征交互转换 |`ft_interaction()`
[–1, 1]区间缩放|`ft_max_abs_scaler()`
[min, max]区间缩放 |`ft_min_max_scaler()`
局部敏感哈希 |`ft_minhash_lsh()`
转换为n-grams |`ft_ngram()`
给定P-norm的归一 |`ft_normalizer()`
One-hot编码 |`ft_one_hot_encoder()`
多项式空间中的特征扩展|`ft_polynomial_expansion()`
映射到分箱的类别型特征|`ft_quantile_discretizer()`
SQL变换 |`ft_sql_transformer()`
使用修正的STD标准化特征 |`ft_standard_scaler()`
过滤停用词|`ft_stop_words_remover()`
映射到标签索引|`ft_string_indexer()`
按空格切分|`ft_tokenizer()`
合并向量为行向量|`ft_vector_assembler()`
为类别型特征建立索引 |`ft_vector_indexer()`
原始特征的子数组|`ft_vector_slicer()`
将词转换为编码|`ft_word2vec()`

## 第6章

### 本地机（主机），云计算和Kubernetes的谷歌趋势
...
第12章中介绍的`stream_generate_test()`函数可以创建本地测试流。此功能独立于Spark连接而工作。下面的示例将在原始子文件夹中创建五个文件。每个文件将在上一个文件完成创建后延迟1秒创建：
...
`stream_generate_test()`默认创建一个单个数值变量的数据框。
...
本说明是根据Kafka官方网站的快速入门页面中的信息形成的。（毫无疑问，在本书出版后不久，新版本的Kafka将面世。）此处是为了给“Kafka”中的例子使用的版本打上“时间戳”：
...
```
    2. 解压*tar*文件，并进入新的文件夹。
```
tar -xzf kafka_2.12-2.2.0.tgz
cd kafka_2.12-2.2.0
```
    3. 启动带有Kafka的Zookeeper服务。
```
...
Javier Luraschi是一名软件工程师，从桌面、web、移动和后端开发到增强现实和深度学习应用程序的技术方面，具有丰富经验。他曾供职于微软研究院和SAP，拥有数学和软件工程双学位。他是`sparklyr`、`cloudml`、`r2d3`、`mlflow`、`tfdeploy`和`kerasjs`的作者。

Kevin Kuo为机器学习和模型部署构建了开源代码库。他曾在多个行业担任数据科学家职位，其中包括保险业，他是一名持照的精算师。Kevin是`mlflow`、`mleap`和`sparkxgb`的作者。他也是一个业余的调酒师和品酒师。

Edgar Ruiz具有部署企业报告和商业智能解决方案的背景。他是多篇文章和博客文章的作者，旨在分享分析见解和数据科学项目的服务器架构。Edgar是`db.rstudio.com`网站的作者和管理员，也是`sparklyr`网站的现任管理员。他还与人合著了`dbplyr`包，并创建了`dbplot`、`tidypredict`和`modeldb`包。

--------

# 结语

《Mastering Spark with R》封面上的动物是一种玫瑰鱼，属于一种深海岩鱼。虽然大多数岩鱼生活在北太平洋，但玫瑰鱼是生活在北大西洋的四种鱼类之一。

幼年玫瑰鱼生活在峡湾附近，而成年玫瑰鱼生活在3280英尺深的水域。成年玫瑰鱼平均有18英寸长，某些也可以达到3英尺。它们是生长缓慢的鱼类，在8或9岁左右达到完全成熟。玫瑰鱼是胎生的。在冬季，雌性会产下50000-350000个卵。玫瑰鱼寿命可达60岁。

玫瑰鱼是一种商业捕捞的鱼类，年捕获量为40至60千吨。人们认为，玫瑰鱼已经被严重过度捕捞了，特别是考虑到需要8或9年才能达到生殖年龄。

奥雷利岛上的许多动物正濒临灭绝，而它们对世界都很重要。

封面插图由Jose Marzan根据Histoire Naturelle的黑白雕刻而成。封面字体是Gilroy Semibold和Guardian Sans。文本字体是Adobe Minion Pro；标题字体是Adobe Myriad Condensed；代码字体是Dalton Maag的Ubuntu Mono。

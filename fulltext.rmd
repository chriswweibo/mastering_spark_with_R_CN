...
* 导入
可以考虑使用R从外部数据源和格式导入数据。例如，当一个文件格式无法获得Spark或其扩展的原生支持时，可以考虑用R代码并借助R包实现分布式自定义解析器。
* 建模
...
尽管Spark及其各种扩展提供了对许多文件格式（csv、JSON、Parquet、AVRO等）的支持，但你会在大规模数据上能需要其他格式。你可以使用`spark_apply()`和许多现有的R包解析这些额外的格式。在本节中，我们将研究如何解析日志文件。使用类似的方法你还可以解析其他文件格式。
...
许多R包提供了需要定义多个参数完成配置和优化的模型。当这些参数的值未知时，我们可以将一列未知参数发放到一组机器上，以找到最佳的参数组合。如果这列参数包含多个要优化的参数，则通常会通过参数A和参数B之间的所有组合进行测试，从而创建一个参数网格。在这个参数网格上搜索最佳参数的过程通常称为网格搜索。
...
我们已经介绍了行和列如何与`spark_apply()`进行交互，接下来我们将继续使用上下文信息，它们有时在处理分布式数据集时需要。
